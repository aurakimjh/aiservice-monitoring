...
> **최종 업데이트**: 2026-03-28 (Session 54 — v1.2+v1.3 + Phase L1+L2 완료)
> **총 건수**: 22건 (해결 20 / 진행중 1 / 접수 1)
...
| # | 심각도 | 제목 | 상태 | 원인 | 해결 | 커밋 |
|---|--------|------|------|------|------|------|
| D-020 | Major | .NET 데모 앱 빌드 실패 (CS8803) | ✅ 해결 | Program.cs에 top-level statement가 class 선언 뒤에 위치 | class 5개를 파일 하단으로 이동 + HttpClientInstrumentation 제거 | `c8cc7fd` |
...
### Session 54 후반 해결
| D-22 | Java Batch 빌드 (Entity import) | ✅ `337bbac` |
| D-23 | Auth middleware prefix match | ✅ `1a2c61f` |
| D-24 | SQLite migrate 다중 statement | ✅ `1a2c61f` |
```
...
| 우선순위 | # | 제목 | 비고 |
|----------|---|------|------|
| P1 | D-008 | Linux 컨테이너 Disk/Process 제한 | 컨테이너 환경 한계 |
| P2 | D-021 | Frontend dev 서버 포트 잔존 | 워크어라운드 존재 |
