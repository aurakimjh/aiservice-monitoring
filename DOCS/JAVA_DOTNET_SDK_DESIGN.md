...
> **문서 버전**: v0.9.0-rc.1
> **작성일**: 2026-03-23 (Session 28) | **최종 업데이트**: 2026-03-28 (v0.9.0-rc.1)
...
## Attach 모드 — 앱 재시작 없이 프로파일링 동적 주입
...
```
JDK tools.jar (또는 Java 9+ com.sun.tools.attach 모듈)의 VirtualMachine 클래스를 통해
실행 중인 JVM에 Java Agent를 동적으로 주입한다. Arthas, Async-Profiler가 동일 방식 사용.

// AITOP Agent (Go) → JVM Attach 브릿지 (Java 헬퍼 프로세스)
exec.Command("java",
  "-cp", "/opt/aitop/plugins/aitop-attach-helper.jar",
  "io.aitop.attach.AttachMain",
  targetPid,
  "/opt/aitop/plugins/aitop-java-agent.jar",
  "mode=attach,interval=10ms,export=otel"
)

// aitop-attach-helper.jar 내부 (Java)
VirtualMachine vm = VirtualMachine.attach(pid);
vm.loadAgent(agentJarPath, agentArgs);
vm.detach();
```

#### Attach 모드에서 활성화되는 기능

| 기능 | Attach 모드 🟢 | Full 모드 🔴 |
|------|:-------------:|:-----------:|
| CPU Sampling (샘플링 기반) | ✅ | ✅ |
| Heap 스냅샷 | ✅ | ✅ |
| Thread 덤프 | ✅ | ✅ |
| HTTP 요청 추적 (Servlet/Spring) | ✅ | ✅ |
| SQL 추적 (JDBC) | ✅ | ✅ |
| 메소드 레벨 계측 (ByteBuddy Retransform) | ⚠️ 제한적¹ | ✅ |
| 클래스 로딩 시점 계측 | ❌ | ✅ |
| 초기화 코드 프로파일링 | ❌ | ✅ |

> ¹ JVM이 이미 로드된 클래스의 Retransform을 허용할 경우에만 가능 (`Can-Retransform-Classes: true`).
> JVM 시작 시 `-XX:+EnableDynamicAgentLoading` (Java 21+) 설정 필요.
...
# Attach 모드 요구사항
requirements:
  jdk: "JDK 8+ 필요 (JRE 불가 — tools.jar 또는 attach API 포함)"
  permissions: "AITOP Agent와 대상 JVM이 동일 OS 사용자이거나 root"
  java21_plus: "--add-opens 또는 --enable-dynamic-agent-loading 플래그 필요"

# agent.yaml 설정
profiling:
  java:
    mode: attach          # attach | full
    attach_timeout_sec: 10
    agent_jar: "/opt/aitop/plugins/aitop-java-agent.jar"
    args: "mode=attach,sampling_interval_ms=10,exporter=otel"
```
...
```
.NET 3.0+부터 제공되는 EventPipe API를 통해 실행 중인 .NET 프로세스에서
진단 이벤트를 스트리밍으로 수집한다. 별도 코드 주입 없이 OS IPC로 연결.

// AITOP Agent (Go) → dotnet-diagnostic IPC 소켓
// Unix: /tmp/dotnet-diagnostic-{pid}
// Windows: \\.\pipe\dotnet-diagnostic-{pid}

연결 흐름:
  DiagnosticsClient client = new DiagnosticsClient(pid);
  EventPipeSession session = client.StartEventPipeSession(providers, requestRundown: false);
  // providers: System.Runtime, Microsoft-Windows-DotNETRuntime 등
  EventPipeEventSource source = new EventPipeEventSource(session.EventStream);
  source.Dynamic.All += (TraceEvent data) => { /* 이벤트 처리 */ };
  source.Process();  // 블로킹 — 별도 고루틴에서 실행
```

#### 수집 가능 이벤트

| EventPipe Provider | 수집 데이터 | Attach 지원 |
|-------------------|------------|:-----------:|
| `System.Runtime` | CPU 사용률, GC 횟수/시간, Thread Pool, 예외 수 | ✅ |
| `Microsoft-Windows-DotNETRuntime` | GC 상세, 메모리 할당, JIT 이벤트 | ✅ |
| `Microsoft-DotNETCore-SampleProfiler` | CPU 샘플링 (스택 트레이스) | ✅ |
| `Microsoft-AspNetCore-Hosting` | HTTP 요청 시작/종료, 상태코드 | ✅ |
| CLR Profiler (ICorProfilerCallback) | 메소드 진입/종료 정밀 계측 | ❌ Full 모드만 |
...
| 예외 카운트 | ✅ | ✅ |
| ASP.NET Core 요청 추적 | ✅ | ✅ |
| 메소드 레벨 계측 (ICorProfilerCallback) | ❌ | ✅ |
| 인라인/JIT 분석 | ❌ | ✅ |
...
# Attach 모드 요구사항
requirements:
  runtime: ".NET Core 3.0+ 또는 .NET 5+"
  ipc_socket: "/tmp/dotnet-diagnostic-{pid} 접근 가능 (동일 사용자 또는 root)"
  windows: "\\.\pipe\dotnet-diagnostic-{pid} Named Pipe 접근"
...
profiling:
  dotnet:
    mode: attach          # attach | full
    attach_timeout_sec: 10
    dll_path: "/opt/aitop/plugins/Aitop.Profiler.dll"   # Full 모드 전용
    providers:
      - name: "System.Runtime"
        keywords: "0xffffffff"
...
### Attach 모드 vs Full Install 모드 비교 (Java / .NET 통합)

| 항목 | 🟢 Attach 모드 (즉시) | 🔴 Full Install 모드 (앱 재시작) |
|------|----------------------|-------------------------------|
| **활성화 방법** | UI 클릭 → 에이전트 자동 주입 | `-javaagent` / CLR Profiler 환경변수 추가 후 재시작 |
| **앱 재시작** | 불필요 | 필요 |
| **활성화 시간** | 수 초 | 다음 재시작 시 |
| **메소드 계측 범위** | 런타임 이후 로드된 클래스 / 샘플링 기반 | 모든 클래스 (로딩 시점 포함) |
| **클래스 초기화 추적** | ❌ | ✅ |
| **성능 오버헤드** | 낮음 (샘플링, ~1~3% CPU) | 중간 (계측 기반, ~3~8% CPU) |
| **프로덕션 권장** | 단기 진단, 긴급 분석 | 상시 APM, 완전 메소드 프로파일링 |
| **Java 설정** | `VirtualMachine.attach(pid)` + `loadAgent()` | `-javaagent:/opt/aitop/aitop-java-agent.jar` |
| **.NET 설정** | `DiagnosticsClient(pid)` EventPipe | `CORECLR_ENABLE_PROFILING=1` + `CORECLR_PROFILER_PATH` |
