...
> **최종 업데이트**: 2026-03-25 (Phase 32 반영 — AGPL-free 인프라 전환 안내 추가, Lite 모드 스토리지 설정 정리)
...
이 프로젝트에서는 Docker로 6개의 서비스(Collector, Prometheus, Jaeger 등)를
...
> Docker    → 모니터링 인프라 (Prometheus, Jaeger 등) 실행
> ```
>
...
| **Jaeger UI** | 트레이스 저장 및 확인 (Apache 2.0) | http://localhost:16686 | 16686 |
| **Grafana Tempo** | 트레이스 백엔드 (**AGPL-3.0 — 개발 참조용**, Phase 30 Jaeger로 대체) | http://localhost:3200 | 3200 |
| **Grafana Loki** | 로그 저장소 (**AGPL-3.0 — 개발 참조용**, Phase 30 stdout/file로 대체) | http://localhost:3100 | 3100 |
| **Grafana** | 시각화 (**AGPL-3.0 — 개발 참조용**, Phase 30 자체 Next.js UI로 대체) | http://localhost:3000 (admin/admin) | 3000 |
...
tempo             grafana/tempo:2.5.0                        Up              0.0.0.0:3200->3200/tcp  # AGPL-3.0 개발 참조용
loki              grafana/loki:3.1.0                         Up              0.0.0.0:3100->3100/tcp  # AGPL-3.0 개발 참조용
grafana           grafana/grafana:11.1.0                     Up              0.0.0.0:3000->3000/tcp  # AGPL-3.0 개발 참조용
```
...
### 트레이스 즉시 확인 (Jaeger UI)

1. 브라우저에서 http://localhost:16686 접속
2. **Service** 드롭다운에서 서비스명 선택
3. **Find Traces** 클릭 → 트레이스 목록 확인

> 로컬에서 SDK를 통해 텔레메트리를 발생시키면 수 초 내에 Jaeger에 반영됩니다.

> **참고**: `docker-compose.yaml`의 Grafana(localhost:3000)는 AGPL-3.0 레거시 개발 도구로, 개발 참조 목적으로만 유지됩니다. 상용 배포에서는 자체 Next.js UI(`docker-compose.commercial.yaml`)를 사용합니다.
...
curl -s http://localhost:16686/api/services | python -c "import sys,json; d=json.load(sys.stdin); print('✅ Jaeger 정상' if d.get('data') is not None else '❌ Jaeger 비정상')"
```
...
### Grafana에 대시보드가 표시되지 않는 경우 (개발 레거시 스택)

> **참고**: Grafana는 AGPL-3.0 개발 참조용 스택으로, 상용 환경에서는 자체 Next.js UI를 사용합니다.
