...
> **문서 버전**: v1.1.0
> **작성일**: 2026-03-22 | **최종 업데이트**: 2026-03-23 (Session 37 — 핵심 가치 명시, Enterprise/Lite 두 배포 모드 비즈니스 시나리오 추가)
> **작성자**: Aura Kim
...
0. [솔루션 핵심 가치 및 배포 모드](#0-솔루션-핵심-가치-및-배포-모드)
...
AITOP은 AI 서비스를 포함하는 엔터프라이즈 환경에서 두 가지 핵심 가치를 제공한다:
...
│     "문제가 생기면 즉시 알려준다"                                      │
│                                                                      │
│     • AI 서비스 포함 전 레이어(OS/WEB/WAS/DB/GPU/LLM) 통합 모니터링  │
│     • 이상 탐지 → 알림 → 인시던트 추적까지 자동화                      │
│     • 평균 탐지 시간(MTTD) 단축: 수동 확인 → 실시간 알림              │
│                                                                      │
│  2. AI 서비스 포함 트랜잭션 프로파일링 → 성능 최적화 인사이트          │
│     ─────────────────────────────────────────────────────           │
│     "어디서 느린지, 왜 느린지 정확히 찾아준다"                         │
│                                                                      │
│     • XLog + HeatMap으로 트랜잭션 분포 시각화                         │
│     • 메소드 콜 트리 (Java/.NET ByteBuddy/CLR 계측)                  │
│     • LLM 체인 Span: Guardrail → Embedding → LLM → Response 구간별  │
│     • GPU 사용률 + TTFT/TPS AI 성능 지표 통합 뷰                     │
│     • "느린 구간 드릴다운 → 원인 → 개선 액션" 플로우 완성             │
│                                                                      │
└─────────────────────────────────────────────────────────────────────┘
```

### 0.2 두 가지 배포 모드 — 비즈니스 활용 시나리오

AITOP은 고객의 상황에 따라 **Enterprise** (상시 운영)와 **Lite** (단기 성능 진단) 두 모드로 배포된다:

#### 모드 A — Enterprise: 상시 운영 모니터링

**시나리오**: AI 서비스를 운영하는 기업이 AITOP을 영구 설치하여 운영 가시성 확보

```
고객 니즈: "AI 서비스가 언제 이상해질지 모른다. 24/7 감시가 필요하다."

배포 흐름:
  설치 (Helm/DEB/RPM) → 에이전트 상주 (systemd) → 장기 데이터 축적
  → 알림 정책 설정 → 이상 탐지 → 인시던트 대응 → SLO 관리

제공 가치:
  • 시스템 이상징후 즉각 알림 (핵심 가치 1)
  • 장기 성능 트렌드 분석 및 용량 계획
  • Fleet 관리 — 수십~수백 대 에이전트 중앙 운영
  • 데이터 장기 보존 (Hot 30d / Warm 90d / Cold 2y)

타겟: Enterprise AI 팀, GPU 클라우드 운영팀
가격: 호스트당 $99/월 (Enterprise 티어)
```

#### 모드 B — Lite: 단기 성능 진단 컨설팅

**시나리오**: 성능 개선 컨설팅 팀이 1주일 투입하여 AI 서비스 성능 병목을 진단

```
고객 니즈: "AI 서비스가 느리다. 어디서 느린지 1주일 안에 찾아달라."

배포 흐름 (1주일 스프린트):
  Day 1: docker-compose up (원클릭 설치)
       → 에이전트 자동 탐지 → 수집 시작
  Day 2~3: 부하 테스트 실행 → XLog/HeatMap으로 응답시간 분포 확인
  Day 4~5: 느린 트랜잭션 프로파일링 → 병목 구간 식별
         → LLM 체인: Guardrail 85ms / Embedding 320ms ← 여기가 병목!
  Day 6~7: 개선 적용 → 부하 테스트 재실행 → 비교 분석
  완료: PDF 진단 보고서 생성 → 고객에게 전달
       docker-compose down -v → 흔적 없이 제거

제공 가치:
  • AI 서비스 포함 트랜잭션 프로파일링 (핵심 가치 2)
  • 부하 테스트 전후 성능 비교 데이터
  • PDF/HTML 진단 보고서 (결과물로 가져갈 수 있음)

설치 요건: Docker만 있으면 됨 (PostgreSQL/S3/Helm 불필요)
타겟: AI 컨설팅 업체, 성능 개선 프로젝트
가격: 프로젝트당 라이선스
```
...
| 항목 | Enterprise | Lite |
|------|-----------|------|
| **기간** | 영구 (상시 운영) | 단기 (1~4주 진단) |
| **핵심 가치** | 이상징후 알림 + 장기 추세 | 트랜잭션 프로파일링 + 성능 개선 |
| **설치** | Helm/DEB/RPM/MSI | `docker-compose up` |
| **설치 요건** | Helm/K8s or OS 패키지 | Docker만 필요 |
| **데이터** | PostgreSQL + S3 | SQLite + 로컬 파일 |
| **보존** | 장기 계층 (Hot/Warm/Cold) | 7일 자동 정리 |
| **제거** | `systemctl disable` | `docker-compose down -v` |
| **결과물** | 대시보드 (영구) | PDF/HTML 보고서 |

> 기술 상세는 [ARCHITECTURE.md §13](./ARCHITECTURE.md#13-배포-모드-아키텍처--enterprise-vs-lite) 및 [AGENT_DESIGN.md §2.3~2.4](./AGENT_DESIGN.md#23-동작-모드)를 참조하세요.
