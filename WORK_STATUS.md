...
> **최종 업데이트**: 2026-03-27 (Session 53 — Phase 42+43+45 완료: OTel 5개 언어 + Frontend Tier 1+2 실데이터)
...
═══════════════════════════════════════════════════════════════════════
  v1.1 실연동 로드맵 진행 현황
═══════════════════════════════════════════════════════════════════════
Phase 41:    인프라 실연동 기반 (Jaeger+Agent+프록시+듀얼모드) ████████████ 100% ✅
Phase 45:    데모 사이트 OTel 계측 (5개 언어)                ████████████ 100% ✅
Phase 42:    Frontend Tier 1 실데이터 전환 (홈·인프라·Fleet·트레이스) ████████████ 100% ✅
Phase 43:    Frontend Tier 2 (서비스·메트릭·로그·알림·토폴로지)      ████████████ 100% ✅
Phase 44:    Frontend Tier 3 (AI·SLO·프로파일링·진단)                ░░░░░░░░░░   0% 📋
...
═══════════════════════════════════════════════════════════════════════
  TO-DO — v1.1 실데이터 연동 + 커스텀 대시보드 + APM 가젯
═══════════════════════════════════════════════════════════════════════

⚠️  현재 상태:
    · Frontend 32개 페이지가 demo-data.ts 하드코딩 데이터만 사용
    · 실제 Prometheus/Jaeger/Agent 데이터가 UI에 표시되지 않음
    · Tempo(AGPL) → Jaeger(Apache-2.0) 교체 필요 (Phase 30 결정 미반영)
    · 데모 데이터 vs 실데이터 구분이 사용자에게 보이지 않음

**실행 순서 (의존성 + 효과 최적화):**

  순서  Phase    내용                                 의존      이유
  ──── ──────── ───────────────────────────────────── ──────── ──────────────────────
   1    41-1    Tempo → Jaeger 교체                  없음     AGPL-free 정합성 최우선
   2    41-2    Agent 빌드 + 설치                    없음     OS 메트릭 수집 시작
   3    41-3    Collection Server 실데이터 프록시     41-1,2   API가 실데이터를 반환하도록
   4    41-4    듀얼 모드 아키텍처 (Demo/Live/Auto)   41-3     모든 페이지 전환의 기반 패턴
   5    45      데모 사이트 OTel 계측 (5개 언어)      41-1     트레이스/메트릭 생성원
   6    42      Frontend Tier 1 (홈·인프라·Fleet·트레이스) 41-4  핵심 4개 페이지 먼저
   7    43      Frontend Tier 2 (서비스·메트릭·로그·알림·토폴로지) 42
   8    46      커스텀 대시보드 시스템                42       Tier 1 완료 후 빌더 착수
   9    47      APM 가젯 8종                          46       대시보드 빌더 위에 가젯 추가
  10    44      Frontend Tier 3 (AI·SLO·프로파일링·진단·나머지) 43

── Phase 41: 인프라 실연동 기반 구축 ─────────────────────────────────────────
[53] Phase 41-1: Tempo → Jaeger 교체 (AGPL-free 정합성)          [██████████] 100%  ✅
     ...
     · Helm values: tempo.enabled=false → jaeger.enabled=true 섹션 추가
[54] Phase 41-2: AITOP Agent 빌드 + 데모 서버 설치               [██████████] 100%  ✅
...
[55] Phase 41-3: Collection Server 실데이터 프록시 구현          [██████████] 100%  ✅
...
[87] Phase 41-4: 듀얼 모드 데이터 소스 아키텍처                   [██████████] 100%  ✅
     · ── 데이터 소스 모드 ──
       - Demo Mode: demo-data.ts 사용 (인프라 불필요, 영업 시연용)
       - Live Mode: Collection Server API만 사용 (Agent/Prometheus 필수)
       - Auto Mode (기본값): API 호출 시도 → 실패 시 Demo 자동 폴백
       - 설정 → 데이터 소스 메뉴에서 전환 가능
     · ── 글로벌 배너 (Demo Mode일 때만 상단 표시) ──
       - "⚠ Demo Mode — 표시된 데이터는 샘플입니다. [Agent 설치 가이드] [닫기]"
       - 노란 배경 배너, 닫기 후에도 상태바에 모드 표시
       - Live Mode에서는 배너 숨김
     · ── 가젯/카드별 데이터 소스 배지 ──
       - 🟢 LIVE — 실데이터 (Agent/Prometheus/Jaeger 연동)
       - 🟡 DEMO — 샘플 데이터 (연동 안 됨)
       - 각 KPI 카드, 차트, 테이블 우측 상단에 소형 배지 표시
       - 페이지 내 혼합 가능 (Fleet은 LIVE, AI 서비스는 DEMO)
     · ── 하단 상태바 데이터 소스 카운트 ──
       - "Data: 3 Live · 5 Demo" 형태로 현재 페이지 데이터 출처 집계
       - 클릭 시 상세 팝업 (어떤 가젯이 LIVE/DEMO인지 목록)
     · ── DataSourceProvider 구현 ──
       - React Context: DataSourceContext (mode, setMode, isLive())
       - useDataSource(apiUrl, demoFallbackFn) 범용 훅
         → Auto: fetch(apiUrl) → 성공 시 {data, source:'live'}
         → Auto: fetch 실패 시 {data: demoFallbackFn(), source:'demo'}
         → Demo: 즉시 {data: demoFallbackFn(), source:'demo'}
         → Live: fetch(apiUrl) → 실패 시 에러 표시 (폴백 없음)
       - 기존 demo-data.ts는 삭제하지 않고 폴백 함수로 유지
     · ── UI Store 확장 ──
       - UIStore.dataSourceMode: 'demo' | 'live' | 'auto'
       - 설정 페이지 (/settings)에 데이터 소스 탭 추가
       - LocalStorage에 저장 (persist)

── Phase 45: 데모 사이트 OTel 계측 완성 ✅ (2026-03-27) ──────────────────────
[70] Phase 45-1: Java OTel Agent 자동 연동                       [██████████] 100%  ✅
     · spring-boot-starter-actuator + micrometer-registry-otlp 추가
     · start-otel.sh / start-otel.bat (javaagent v2.12.0 자동 다운로드)
     · Spring Boot + Spring Batch 트레이스/메트릭 → OTLP gRPC :4317
[71] Phase 45-2: Python OTel SDK 계측                            [██████████] 100%  ✅
     · opentelemetry-sdk + instrumentation-fastapi/psycopg/redis/httpx
     · main.py / rag_service.py / guardrail_service.py / tasks.py (Celery)
     · OTEL_EXPORTER_OTLP_ENDPOINT 환경변수 기반 활성화 (HTTP :4318)
[72] Phase 45-3: Go OTel SDK 통합                                [██████████] 100%  ✅
     · otelgin 미들웨어 (Gin HTTP 계측)
     · otelpgx (PostgreSQL 쿼리 트레이스)
     · redisotel (Redis 명령 트레이스 + 메트릭)
     · otelhttp (Outbound HTTP 계측) → OTLP gRPC :4317
[73] Phase 45-4: .NET OTel 계측 강화                             [██████████] 100%  ✅
     · Metrics: AddRuntimeInstrumentation + AddProcessInstrumentation
     · Tracing: AddEntityFrameworkCoreInstrumentation 추가
     · WithMetrics 파이프라인 추가 (ASP.NET + HttpClient + Runtime)
[74] Phase 45-5: Node.js OTel Metrics + 문서                    [██████████] 100%  ✅
     · OTLPTraceExporter + OTLPMetricExporter 명시적 구성
     · PeriodicExportingMetricReader (10s interval)
     · DEMO_SETUP.md: 5개 언어 OTel 실행 가이드 전면 개정
     · README.md: OTel 계측 매트릭스 추가

── Phase 42: Frontend 실데이터 전환 (Tier 1 — 핵심) ✅ (2026-03-27) ──────
[56] Phase 42-1: 홈 대시보드 (/) 실데이터 전환                   [██████████] 100%  ✅
     · useDataSource('/realdata/overview') → Agent 수, CPU, 백엔드 상태
     · KPICard badge prop 추가 → DataSourceBadge(LIVE/DEMO) 자동 표시
     · Service Health Map: live 시 agent 상태 기반 / demo 시 하드코딩
     · 30초 자동 갱신, demo fallback 유지
[57] Phase 42-2: 인프라 뷰 (/infra) 실데이터 전환               [██████████] 100%  ✅
     · useDataSource('/realdata/hosts') → Agent Heartbeat 기반 호스트 목록
     · transformHosts(): API 응답 → Host 타입 매핑 (status/cpu/agent)
     · 페이지 제목 옆 DataSourceBadge 표시
     · 30초 자동 갱신, demo fallback getProjectHosts()
[58] Phase 42-3: 에이전트 Fleet 콘솔 (/agents) 검증             [██████████] 100%  ✅
     · 이미 useFleet() 훅으로 API-first + demo fallback 구현 완료
     · LIVE/DEMO 배지 + Wifi/WifiOff 아이콘 표시 확인
     · 추가 작업 불필요 (Phase 41-4에서 완성)
[59] Phase 42-4: 트레이스 뷰 (/traces) 실데이터 전환            [██████████] 100%  ✅
     · useDataSource('/proxy/jaeger/services') → 서비스 목록 실시간 조회
     · useDataSource('/proxy/jaeger/traces') → Jaeger 트레이스 검색
     · transformJaegerTraces(): Jaeger span → Transaction 타입 변환
     · XLog/HeatMap/Split 모든 뷰 실데이터 지원, demo fallback 유지
     · DataSourceBadge 페이지 헤더에 표시

── Phase 43: Frontend 실데이터 전환 (Tier 2 — 모니터링) ✅ (2026-03-27) ──
[60] Phase 43-1: 서비스 뷰 (/services)                           [██████████] 100%  ✅
     · useDataSource('/proxy/jaeger/services') → 서비스 목록 실데이터
     · transformJaegerToServices(): Jaeger 서비스명 → Service 타입
     · useDataSource('/proxy/jaeger/dependencies') → 토폴로지 실데이터
     · DataSourceBadge 페이지 헤더에 표시
[61] Phase 43-2: 메트릭 탐색기 (/metrics)                        [██████████] 100%  ✅
     · queryPrometheus(): /proxy/prometheus/query_range 직접 실행
     · MetricPanel: Prometheus 우선 → 실패 시 demo executeMetricQuery() 폴백
     · useDataSource('/proxy/prometheus/status') → 연결 상태 표시
     · DataSourceBadge 페이지 헤더에 표시
[62] Phase 43-3: 로그 뷰 (/logs)                                 [██████████] 100%  ✅
     · useDataSource('/logs?service=...&level=...') → 로그 검색 API
     · useDataSource('/logs/patterns') → 로그 패턴 분석
     · 15초 자동 갱신, demo fallback generateLogEntries()
[63] Phase 43-4: 알림 뷰 (/alerts)                               [██████████] 100%  ✅
     · useDataSource('/alerts/policies') → 알림 정책 실데이터
     · useDataSource('/alerts/incidents') → 인시던트 목록 (15초 갱신)
     · useDataSource('/alerts/channels') → 알림 채널 설정
     · DataSourceBadge 페이지 헤더에 표시
[64] Phase 43-5: 토폴로지 (/topology)                            [██████████] 100%  ✅
     · useDataSource('/proxy/jaeger/dependencies') → 서비스 의존성
...
[79] Phase 47-1: TPS 가젯                                         [░░░░░░░░░░]   0%  📋
     · 실시간 TPS — 5초 집계, 최근 10분, area 차트
     · 모아보기(SUM) ↔ 개별보기 전환 가능
     · Prometheus: rate(http_server_request_duration_seconds_count[5s])
[80] Phase 47-2: 금일 TPS 가젯 (일간 비교)                        [░░░░░░░░░░]   0%  📋
     · 오늘 TPS 시간대별 추이 (X 00:00~24:00)
     · 비교 날짜 선택 — 비교군: 연한 회색 배경 영역 / 오늘: 파란 실선
     · SUM 고정 / 툴팁: "현재 1,250 TPS / 어제 980 TPS (+27.6%)"
[81] Phase 47-3: 금일 사용자 수 가젯 (일간 비교)                  [░░░░░░░░░░]   0%  📋
     · 오늘 동시접속 사용자 수 시간대별 추이
     · 비교군: 연한 회색 배경 / 오늘: 녹색 실선 / SUM 고정
     · 사용자 카운팅: 5분 이내 IP 기반 (쿠키 선택 가능)
[82] Phase 47-4: 평균 응답시간 가젯                                [░░░░░░░░░░]   0%  📋
     · 5초 간격 평균 응답시간 (ms) — 최근 10분
     · 임계치 참조선 (warning 노란 점선 / critical 빨간 점선)
     · 모아보기(SUM) ↔ 개별보기 전환 가능
[83] Phase 47-5: 액티브 트랜잭션 가젯                              [░░░░░░░░░░]   0%  📋
     · 진행 중 트랜잭션 수 — 5초 갱신, area 차트 + 큰 숫자
     · 급증 시 색상 전환 (파랑 → 노랑 → 빨강)
     · 모아보기 ↔ 개별보기 전환 가능
[84] Phase 47-6: 액티브 스테이터스 가젯                            [░░░░░░░░░░]   0%  📋
     · 상태별 카운트: METHOD(파) / SQL(주) / HTTPC(보) / DBC(빨⚠) / SOCKET(빨⚠)
     · DBC/SOCKET ≥ 1 지속 시 경고 (깜빡임 + "Connection Pool 고갈 의심")
     · 모아보기: 전체 합산 / 개별보기: 인스턴스별 테이블
[85] Phase 47-7: 액티브 트랜잭션 스피드 차트                       [░░░░░░░░░░]   0%  📋
     · 3단 플로우: RPS(좌) → 처리중(중앙 게이지) → TPS(우)
     · 실시간 애니메이션 / RPS > TPS 지속 시 빨강 전환 (적체)
     · SUM 고정
[86] Phase 47-8: 동시접속 사용자 가젯                              [░░░░░░░░░░]   0%  📋
     · 실시간 사용자 수 — 5초마다 최근 5분 IP 카운팅
     · 최근 30분 area 차트 + 현재 값 큰 숫자
     · 모아보기 ↔ 개별보기 전환 가능

── Phase 44: Frontend 실데이터 전환 (Tier 3 — AI/고급, 마지막) ─────────────
[65] Phase 44-1: AI 서비스 뷰 (/ai)                              [░░░░░░░░░░]   0%  📋
     · LLM 메트릭: Prometheus aiservice_llm_* 쿼리
     · GPU 클러스터: Agent GPU Collector / RAG: Jaeger span / 가드레일: Prometheus
[66] Phase 44-2: SLO 뷰 (/slo)                                   [░░░░░░░░░░]   0%  📋
     · SLO CRUD API / Error Budget (Prometheus recording rule) / Burn Rate
[67] Phase 44-3: 프로파일링 (/profiling)                          [░░░░░░░░░░]   0%  📋
     · StorageBackend 조회 / FlameGraph SVG / Jaeger traceId 링크
[68] Phase 44-4: 진단 (/diagnostics)                              [░░░░░░░░░░]   0%  📋
     · Evidence API 실데이터 / ZIP → JSON / IT↔AI 교차 분석
[69] Phase 44-5: 나머지 페이지                                    [░░░░░░░░░░]   0%  📋
     · /anomalies, /copilot, /golden-signals, /rum, /runtime/*, /database
     · /batch, /cloud, /pipelines, /business, /marketplace
```
...
│                                                                             │
│  ── v1.1 실데이터 연동 + 커스텀 대시보드 + APM 가젯 ──────────────────────  │
│                                                                             │
│  ①  Phase 41-1 (Tempo→Jaeger) ──┐                                        │
│  ②  Phase 41-2 (Agent 빌드) ────┤        ✅ 전체 완료                     │
│                                  ▼                                         │
│  ③  Phase 41-3 (CS 실데이터 프록시) ──► ④ Phase 41-4 (듀얼 모드 아키텍처)│
│                                              │                             │
│  ⑤  Phase 45 (OTel 5개 언어) ✅ ────────────┤                            │
│                                              ▼                             │
│  ⑥  Phase 42 (Tier1: 홈·인프라·Fleet·트레이스) ✅ ──┬────────────────── │
│                                                      │                     │
│  ⑦  Phase 43 (Tier2: 서비스·메트릭·로그·알림) ◄─────┘  ← 다음 작업      │
│         │                                                                  │
│  ⑧  Phase 46 (커스텀 대시보드, ⑥후) ──► ⑨ Phase 47 (APM 가젯 8종)      │
│                                         · TPS / 금일TPS / 금일사용자       │
│                                         · 응답시간 / 액티브트랜잭션         │
│                                         · 스테이터스 / 스피드 / 동시접속     │
│                                                                             │
│  ⑩  Phase 44 (Tier3: AI·SLO·프로파일링·진단·나머지, ⑦후)               │
└─────────────────────────────────────────────────────────────────────────────┘
```
