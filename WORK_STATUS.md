...
> **최종 업데이트**: 2026-03-22 (Session 18 — Phase 15-4-4 Fleet UI 연동 완료 → Phase 15 MVP 100%)
...
═══════════════════════════════════════════════════════════════════════
Phase 15: Agent MVP (F)        [██████████] 100%  ✅  (Agent Core ✅ + OS Collector ✅ + AI Collector ✅ + Fleet UI 연동 ✅)
Phase 16: Agent GA (G)         [████████░░]  75%  🔄  (IT Collector ✅ + Remote CLI ✅ + OTA ✅ + Fleet 관리 콘솔 📋)
...
## 작업 로드맵 (2026-03-20 기준)

### Phase 7-9 재평가

> Phase 10-11에서 UI가 Grafana → Next.js로 완전 교체됨에 따라 Phase 7-9의 범위 재정의 필요.

| Phase | 원래 범위 | 변경 사유 | 판단 |
|-------|----------|----------|------|
| **7** (E2E 검증) | Grafana 대시보드 + Jaeger + Prometheus 검증 | Grafana 프론트엔드가 Next.js UI로 교체 → 대시보드 검증 항목 절반 무효 | **재설계 후 진행** — 백엔드(OTel/Tempo/Loki) 검증은 유효, 프론트엔드 검증은 새 UI 기준으로 재작성 |
| **8** (K8s 배포) | Helm으로 모니터링 스택 배포 | 백엔드 Helm은 유효하나 Next.js 프론트엔드 Dockerfile + Helm 차트 추가 필요 | **UI 완성 후 통합 배포** |
...
```
── 현재 (UI 기능 완성) ──────────────────────────────────────────
Phase 12: AI 네이티브 UI       📋  ← AI 핵심 차별점, 컴포넌트 모멘텀 활용
  12-1. AI 서비스 개요 + 목록
  12-2. LLM 성능 대시보드
  12-3. GPU 클러스터 뷰
  12-4. RAG 파이프라인 뷰
...
Phase 13: 에이전트 통합 + 알림  📋  ← AITOP 에이전트 연동, 인시던트 관리
Phase 14: 고도화               📋  ← 커스텀 대시보드, SLO 뷰, 비용 분석

── UI 완성 후 (인프라 + 배포) ───────────────────────────────────
Phase 7': E2E 검증 (재설계)    📋 🔧  ← 새 UI 기준 시나리오, 백엔드 통합
Phase 8': 통합 배포            📋 🔧  ← Next.js + 백엔드 Helm 통합, K8s
Phase 9 : SLO 튜닝            📋 🔧  ← 프로덕션 운영 데이터 필요
```
...
Phase 12: AI 네이티브 UI                                   ✅ 완료
Phase 13: 에이전트 통합 + 알림                             ✅ 완료
Phase 14: 고도화                                           ✅ 완료 (대시보드빌더+SLO+비용+Executive+노트북+멀티테넌트+i18n+성능)

═══ 검증 & 배포 (UI 완성 후) ═══
Phase 7': E2E 통합 검증 (재설계)                          📋 → 새 UI 기준 재작성 필요
Phase 8': 통합 배포 (Next.js + 백엔드)                    📋 🔧 수작업
Phase 9 : SLO 튜닝                                      📋 🔧 → 프로덕션 운영 데이터 필요
```
...
> **예상 기간**: 6주
> **산출물**: Go 에이전트 바이너리 + Collection Server + Fleet 기본 UI
...
| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 16-3-1 | Agent PTY Service | Go PTY 할당 (syscall/ptmx), 세션 Manager 구현 | ✅ |
| 16-3-2 | TerminalService Proto | SessionManager + InputEvent/OutputEvent/ResizeEvent | ✅ |
| 16-3-3 | Backend WebSocket Proxy | RemoteCommand terminal.open/input/close 처리 | ✅ |
...
| 16-3-6 | 감사 로그 | AuditLogger NDJSON, SESSION_OPEN/INPUT/BLOCKED/SESSION_CLOSE | ✅ |
| 16-3-7 | 세션 관리 | idle 타임아웃, 최대 세션 기간, 동시 세션 제한 | ✅ |

### 16-4. OTA 업데이트 + Fleet 관리 — 7~8주차

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 16-4-1 | Update Manager | 에이전트 바이너리 버전 관리, SHA-256 체크섬 검증, atomic replace | ✅ |
| 16-4-2 | 단계 배포 (Staged Rollout) | canary(1%) → 10% → 50% → 100%, agentID 해시 기반 버킷 배정 | ✅ |
| 16-4-3 | 자동 롤백 | health degradation 감지 시 이전 안정 버전 자동 복원 | ✅ |
| 16-4-4 | Fleet 관리 콘솔 완성 | 그룹 관리, 플러그인 배포 UI, 수집 스케줄 설정, 업데이트 현황 | 📋 |
| 16-4-5 | 수집 스케줄링 | cron 기반 정기 수집 + OTA 일일 점검 스케줄 (03:00) | ✅ |
| 16-4-6 | collect-export 모드 | `--mode=collect-export` 오프라인 NDJSON 내보내기 | ✅ |
...
| `DOCS/UI_DESIGN.md` | ✅ v1.0.0 | 통합 모니터링 UI 설계 (상용 솔루션 수준) |
| `DOCS/AGENT_DESIGN.md` | ✅ v1.0.0 | **신규** — AITOP Agent 상세 설계 (Collector + 권한 + Fleet + CLI + 저장소) |
...
> **브랜치**: `claude/pedantic-hopper`
> **작업 범위**: Phase 16 Agent GA — IT Collector + Remote CLI + OTA Updater

### 완료된 작업

- [x] **`agent/internal/collector/web/web_collector.go`** — WEB Collector (16-1-1)
  - AutoDetect: nginx/apache/httpd/IIS 프로세스 + 포트 탐지
  - Collect: 서버 버전/설정 파싱 (nginx.conf/apache2.conf), SSL 인증서 만료일 (TLS dial), nginx_status 메트릭
  - `web_collector_test.go`: interface, auto-detect, config 파싱, SSL port 추출 테스트

- [x] **`agent/internal/collector/was/was_collector.go`** — WAS Collector (16-1-2)
  - AutoDetect: /proc 기반 Java 프로세스 탐지 (Tomcat/Spring Boot/JBoss/Jetty)
  - Collect: JVM 설정 파싱 (-Xms/-Xmx/-Xss/-XX:+UseG1GC/JMX), GC 로그 분석 (JDK 8/11+ 양식), Thread Dump (jcmd/jstack)
  - `was_collector_test.go`: JVM 파싱, GC pause 추출, Java 프로세스 분류 테스트

- [x] **`agent/internal/collector/db/db_collector.go`** — DB Collector (16-1-3)
  - AutoDetect: postgres/mysqld/mongod/ora_pmon 프로세스 + 포트 체크
  - Collect: 서버 버전, 커넥션 상태 (pg_stat_activity/SHOW STATUS/mongosh), 설정 파라미터 (pg_settings/postgresql.conf/SHOW VARIABLES), 슬로우 쿼리 (pg_stat_statements/performance_schema)
  - CLI 폴백: psql/mysql/mongosh 미인증 시 CLI 기반 수집
  - `db_collector_test.go`: interface, port check, config struct 테스트

- [x] **`agent/internal/collector/it/register.go`** — IT Collector 등록 헬퍼
  - `RegisterAll(registry)`: OS + WEB + WAS + DB 일괄 등록

- [x] **`agent/internal/shell/shell.go`** — Remote CLI Service (16-3-1~7)
  - `Manager`: 세션 생명주기 관리 (OpenSession/SendInput/Resize/Close), MaxSessions 제한, idle/max 타임아웃
  - `Session`: PTY 기반 인터랙티브 셸 (`/dev/ptmx` + TIOCSWINSZ), 입출력 펌프, goroutine 기반 비동기 처리
  - `RBACFilter`: admin/sre 역할 기반 접근 제어, 기본 15개 위험 명령 차단 (rm -rf /, shutdown, reboot 등)
  - `AuditLogger`: NDJSON 형식 감사 로그 (SESSION_OPEN/INPUT/BLOCKED/SESSION_CLOSE)
  - `shell_unix.go`: Linux PTY 구현 (syscall 기반, TIOCSWINSZ)
  - `shell_windows.go`: Windows 빌드 no-op 스텁
  - `shell_test.go`: RBAC 필터, 감사 로그, 세션 관리, 역할 검증 테스트

- [x] **`agent/internal/updater/updater.go`** — OTA Updater (16-4-1~3)
  - `Manager`: 버전 확인 → 바이너리 다운로드 → SHA-256 검증 → Staged Rollout → atomic replace
  - Staged Rollout: canary(1%) → 10% → 50% → 100%, agentID SHA-256 해시 기반 버킷 배정 (결정론적)
  - 자동 롤백: health 체크 연속 3회 실패 시 .prev 바이너리 복원
  - `updater_test.go`: semver 파싱, IsNewer, 롤아웃 퍼센타일, 체크섬 검증, 상태 전환 테스트

- [x] **`agent/cmd/aitop-agent/main.go`** — 통합 업데이트
  - `itcol.RegisterAll()` + `aicol.RegisterAll()` 분리 등록
  - `shell.Manager` 조건부 초기화 (RemoteShell.Enabled)
  - `updater.Manager` 초기화 (full 모드 + 서버 URL 있을 때)
  - OTA 스케줄 등록 (`0 3 * * *` 일일 점검)
  - `terminal.open/input/close` 원격 커맨드 핸들러 추가
  - 종료 시 shell 세션 graceful close

### 테스트 결과

```
ok  github.com/aurakimjh/aiservice-monitoring/agent/internal/collector/web   2.7s
ok  github.com/aurakimjh/aiservice-monitoring/agent/internal/collector/was   1.0s
ok  github.com/aurakimjh/aiservice-monitoring/agent/internal/collector/db    0.9s
ok  github.com/aurakimjh/aiservice-monitoring/agent/internal/shell           0.6s
ok  github.com/aurakimjh/aiservice-monitoring/agent/internal/updater         0.6s
ok  [전체 기존 테스트 유지]
```
...
| 파일 | 설명 |
|------|------|
| `agent/internal/collector/web/web_collector.go` | WEB Collector |
| `agent/internal/collector/web/web_collector_test.go` | WEB 테스트 |
| `agent/internal/collector/was/was_collector.go` | WAS Collector |
| `agent/internal/collector/was/was_collector_test.go` | WAS 테스트 |
| `agent/internal/collector/db/db_collector.go` | DB Collector |
| `agent/internal/collector/db/db_collector_test.go` | DB 테스트 |
| `agent/internal/collector/it/register.go` | IT RegisterAll |
| `agent/internal/shell/shell.go` | Remote Shell Service |
| `agent/internal/shell/shell_unix.go` | Unix PTY 구현 |
| `agent/internal/shell/shell_windows.go` | Windows 스텁 |
| `agent/internal/shell/shell_test.go` | Shell 테스트 |
| `agent/internal/updater/updater.go` | OTA Updater |
| `agent/internal/updater/updater_test.go` | Updater 테스트 |
