...
> **최종 업데이트**: 2026-03-22 (Session 18 — Phase 15-4 Heartbeat+StateMachine+Collection Server, 15-5 Prometheus RemoteWrite+collect-only+Deploy 완료)
...
═══════════════════════════════════════════════════════════════════════
Phase 15: Agent MVP (F)        [████████░░]  85%  🔄  (Agent Core ✅ + OS Collector ✅ + AI Collector ✅ + StateMachine ✅ + Heartbeat ✅ + Collection Server ✅ + Prometheus ✅ + Deploy ✅)
Phase 16: Agent GA (G)         [░░░░░░░░░░]   0%  📋  (전체 Collector + 원격 CLI + OTA + Fleet 관리 완성)
...
> **완료 일자**: 2026-03-22 | **구현 파일**: `agent/internal/statemachine/`, `agent/internal/transport/`, `agent/cmd/collection-server/`

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 15-4-1 | Heartbeat 프로토콜 | `transport/heartbeat.go` — 30초 간격 HTTP POST, 원격 명령 채널 반환 | ✅ |
| 15-4-2 | 에이전트 상태 머신 | `internal/statemachine/state_machine.go` — registered→approved→healthy→degraded→offline + quarantine/retire | ✅ |
| 15-4-3 | Fleet 기본 REST API | `cmd/collection-server/` — GET /agents, GET /agents/{id}, POST /agents/{id}/collect | ✅ |
| 15-4-4 | Fleet UI 연동 | 기존 Phase 13 Agent Fleet 화면에 실데이터 바인딩 | 📋 |
| 15-4-5 | 권한 리포트 API | `GET /api/v1/agents/{id}/privileges` — Collection Server에 구현 완료 | ✅ |
...
| 15-5-8 | Prometheus Remote Write | `transport/prometheus.go` — gzip text/plain exposition format, CollectResultToMetrics 변환 | ✅ |
| 15-5-9 | collect-only 모드 | `main.go` 업데이트 — `--mode=collect-only` → 수집 → HTTPS flush → 종료 | ✅ |
| 15-5-10 | Agent 설치 패키지 | `deploy/systemd/aitop-agent.service` + `deploy/install.sh` + Makefile `package` 타겟 | ✅ |
...
> **브랜치**: `claude/sharp-greider`
> **작업 범위**: Phase 15-4 (StateMachine + Heartbeat + Collection Server) + Phase 15-5 (Prometheus RemoteWrite + collect-only + Deploy)

### 완료된 작업

- [x] **`agent/internal/statemachine/state_machine.go`** — 에이전트 생명주기 상태 머신
  - 상태: registered → approved → healthy → degraded → offline (+ upgrade, quarantine, retire)
  - 전환 이벤트: approve, checkin_ok, degraded, recovered, missed_heartbeat, quarantine, release, retire, upgrade_*
  - `HeartbeatAck()` / `HeartbeatMissed()` 헬퍼 (MaxMissedBeats=3 초과 시 offline 전환)
  - Observer 패턴 `OnChange()` 핸들러, `History()` 스냅샷
  - 테스트: **10개 PASS**
- [x] **`agent/internal/transport/heartbeat.go`** — Heartbeat HTTP 송신 + HTTPS collect 클라이언트
  - `HeartbeatSender.Run()`: 30초 간격 POST /api/v1/heartbeat
  - `HeartbeatSender.Send()`: 단건 전송 + 명령 채널 자동 디스패치
  - `HTTPClient.SendCollectResult()`: collect-only 모드 HTTPS 전송
  - 테스트: **5개 PASS**
- [x] **`agent/internal/transport/prometheus.go`** — Prometheus Remote Write
  - gzip + text/plain exposition format (no protobuf 의존)
  - `CollectResultToMetrics()`: CollectResult JSON → `aitop_{collector}_{field}` 메트릭 변환
  - VictoriaMetrics / Grafana Mimir / Thanos 호환
  - 테스트: **7개 PASS**
- [x] **`agent/cmd/collection-server/main.go`** — Collection Server MVP (in-memory Fleet)
  - `POST /api/v1/heartbeat` — 에이전트 등록 + 상태 반환
  - `GET /api/v1/agents` — Fleet 목록
  - `GET /api/v1/agents/{id}` — 에이전트 상세
  - `POST /api/v1/agents/{id}/collect` — 즉시 수집 트리거
  - `GET /api/v1/agents/{id}/privileges` — 권한 리포트
  - `GET /healthz` — 헬스체크
  - 서버사이드 상태 머신: registered → approved → healthy / degraded
  - 테스트: **9개 PASS**
- [x] **`agent/cmd/aitop-agent/main.go`** 업데이트
  - 상태 머신 통합 (server.url 설정 시 자동 approve)
  - Heartbeat 루프 고루틴 시작 (full 모드)
  - 원격 명령 처리 고루틴 (collect 명령 지원)
  - collect-only 모드: 수집 → buffer → HTTPS flush → 종료
- [x] **`agent/deploy/systemd/aitop-agent.service`** — systemd 서비스 파일
  - User/Group: aitop, MemoryMax: 200M, CPUQuota: 20%
  - NoNewPrivileges, ProtectSystem=strict, PrivateTmp
- [x] **`agent/deploy/install.sh`** — 설치 스크립트
  - 시스템 사용자 생성, 디렉토리 구성, 바이너리 복사
  - 기본 agent.yaml 생성, systemd 서비스 등록 및 시작
- [x] **Makefile** 업데이트: `build-collection-server`, `package` 타겟 추가
- [x] **`go build ./...` 성공** + **전체 테스트 PASS** (신규 31개 포함)
- [x] **`WORK_STATUS.md`** 업데이트 — Phase 15-4/15-5 완료 반영 (85%)
