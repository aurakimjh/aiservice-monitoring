...
> **최종 업데이트**: 2026-03-27 (Session 52 — 실데이터 연동 Phase 41~45 + 커스텀 대시보드 Phase 46 + APM 가젯 Phase 47 TO-DO 추가)
...
Phase 40:    출시 전 Critical (RUM·Golden·Runtime·DB)    ████████████ 100% ✅
Phase 8':    Kubernetes 통합 배포 (Helm+kind+DEB/RPM)   ████████████ 100% ✅
Phase 9':    SLO 튜닝 + Tail Sampling + 릴리스 승인   ████████████ 100% ✅
...
[15] Phase 31-1: 내장(🔧) 진단 항목 Go 구현 (ConfigEvidence·LogEvidence·EOSEvidence)  [██████████] 100%  ✅
[16] Phase 31-2: 스크립트(📜) 실행 프레임워크 (Script Executor·스케줄러·Backend 연동)  [██████████] 100%  ✅
[17] Phase 31-3: 수동(🖐️) 트리거 방식 + 고급 진단 (Security·APM·CrossAnalysis·Full)   [██████████] 100%  ✅
[18] Phase 31-4: Java Agent EOL (기능 동등성 검증·마이그레이션)                        [██████████] 100%  ✅
...
── 중앙 플러그인 배포 ✅ ──────────────────────────────────────────────────
[26] Phase 33-1: Plugin Manager + File Watcher (에이전트 측)   [██████████] 100%  ✅
[27] Phase 33-2: Plugin Registry + Deploy API (서버 측)        [██████████] 100%  ✅
[28] Phase 33-3: 배포 전략 (즉시/단계/예약) + 자동 롤백         [██████████] 100%  ✅
[29] Phase 33-4: Fleet Console Plugin UI + 배포 이력           [██████████] 100%  ✅

── perf/eBPF 시스템 프로파일링 ✅ ─────────────────────────────────────────
[35] Phase 35-1: perf/eBPF Collector 모듈 — 에이전트 측        [██████████] 100%  ✅
     · perf record → perf script 커널+유저 통합 스택 트레이스 수집
     · eBPF on-CPU / off-CPU / memory 프로파일러 (bpftrace 폴백)
     · 99Hz 샘플링, 30s 기본 수집, PID 또는 시스템 전체 대상
     · folded stack (gzip) → Collection Server 전송
     · 권한 검증: CAP_BPF + CAP_PERFMON (또는 CAP_SYS_ADMIN)
     · 신규 에러 코드: EBPF_CAP_MISSING, EBPF_KERNEL_VERSION, PERF_MAP_NOT_FOUND 등
[36] Phase 35-2: 언어별 JIT 심볼 해석 연동                     [██████████] 100%  ✅
     ...
     · .NET: crossgen2 PerfMap (DOTNET_EnablePerfMap 환경변수)
[37] Phase 35-3: Collection Server — 플레임그래프 생성 엔진      [██████████] 100%  ✅
     · folded stack → SVG / JSON 변환 (Brendan Gregg 알고리즘 Go 구현)
     · 타입별 렌더링: on-CPU(주황) / off-CPU(파랑) / memory(녹색) / mixed
     · diff 플레임그래프: A vs B 비교 (빨강=증가 / 파랑=감소)
     · API: GET /api/v1/profiling/flamegraph, stacks, POST trigger, system/upload
     · StorageBackend 저장 (perf-profiles/ 키 패턴)
[38] Phase 35-4: 인터랙티브 플레임그래프 UI                     [██████████] 100%  ✅
     · /profiling/system 독립 뷰 — 에이전트/타입/시간 범위 선택 + diff 모드
     · 확대/축소, 함수 검색, hover 시 비율·샘플수 표시, 우클릭 컨텍스트 메뉴
     · 색상: on-CPU(주황) / off-CPU(파랑) / memory(녹색) / diff(빨강·파랑)
     · SVG 다운로드, JSON 내보내기, folded stack 원본 다운로드
     · KPI 카드, 프로필 목록 테이블, 심볼 해석률 표시

── 배치 모니터링 ─────────────────────────────────────────────────────────
[39] Phase 36: 배치 프로세스 자동 감지 + 프로세스 레벨 수집        [██████████] 100%  ✅
     · cron/systemd/WTS 자식 프로세스 자동 감지 (4단계 규칙)
     · /proc/{PID} 폴링 (stat/io/status) + Windows WMI 스텁
     · BatchExecution 생명주기 (DETECTED→RUNNING→COMPLETED/FAILED)
     · Spring Batch + Airflow REST API 프레임워크 연동
     · Collection Server 7개 REST API + 24개 데모 실행 이력
[40] Phase 37: 배치 런타임 프로파일링                              [██████████] 100%  ✅
     · Java: async-profiler JDBC 훅 SQL Top-N + 메소드 Top-N + JVM GC 메트릭
     · Python: py-spy folded stack → 함수 Top-N + 스레드 덤프
     · .NET: EventPipe EF Core 쿼리 + dotnet-counters GC/ThreadPool
     · Go: pprof CPU/heap/goroutine 자동 수집
     · perf/eBPF: 배치 PID 타겟 on-CPU/off-CPU 플레임그래프
     · Collection Server 8개 API + 6개 배치 실행 데모 프로파일
[41] Phase 38: 배치 대시보드 뷰                                    [██████████] 100%  ✅
     · /batch 작업 목록 (KPI 카드, 상태 배지, 언어/상태 필터, 타임라인)
     · /batch/[name] 잡 상세 (이력 바 차트, 실행 테이블)
     · /batch/executions/[id] 실행 상세 (5탭: Overview/SQL/Methods/Flamegraph/Runtime)
     · /batch/xlog 산점도 (ECharts scatter, SLA 임계선, 클릭→상세)
     · /batch/alerts 알림 규칙 CRUD + 이력 (4개 데모 규칙, 5개 알림 이력)
     · Collection Server 6개 알림 API + 사이드바/i18n 통합

── Runtime Attach 프로파일링 (Phase 33 플러그인과 연계, 병렬 진행 가능) ────
[30] Phase 34-1: Runtime Attach 모듈 — Java JVM Attach API     [██████████] 100%  ✅
...
[31] Phase 34-2: Runtime Attach 모듈 — Python py-spy            [██████████] 100%  ✅
     · py-spy 바이너리 플러그인 배포 연계
     · PID 기반 외부 스택 샘플링 + FlameGraph SVG 생성
     · SYS_PTRACE capability 사전 검증
[32] Phase 34-3: Runtime Attach 모듈 — .NET EventPipe           [██████████] 100%  ✅
     · DiagnosticsClient IPC 소켓 연결 (Unix/Windows)
     · EventPipe Provider 설정 (System.Runtime, DotNETRuntime)
     · .nettrace → OTel Metric 변환
[33] Phase 34-4: Runtime Attach 모듈 — Node.js CDP / Go pprof   [██████████] 100%  ✅
     · Node.js: SIGUSR1 → V8 Inspector CDP WebSocket 연결
     · Go: /debug/pprof/profile HTTP 폴링
[34] Phase 34-5: 프로파일링 플러그인 배포 UI (Phase 33 연계)     [██████████] 100%  ✅
     · /agents/{id}/profiling 화면 — 모드 선택 (🟢 Attach / 🔴 Full Install)
     · "프로파일링 플러그인 배포" 원클릭 → 자동 Attach 워크플로
     · Attach 상태 실시간 표시 (활성/비활성/오류)

── JDK 21 Virtual Thread 모니터링 ─────────────────────────────────────────
[42] Phase 39-1: JFR Virtual Thread 이벤트 구독 (Agent Attach 확장)  [██████████] 100%  ✅
     · VirtualThreadDetector: JDK 버전 감지 + 21+ 자동 활성화
     · JFRSubscription XML (6개 이벤트: Pinned/SubmitFailed/Start/End/Park/Unpark)
     · java_attach.go: Attach 시 vt-subscribe 실행 + javaSession VirtualThreadInfo 저장
     · VTMetricsToOTelGauges: 11개 OTel Gauge 메트릭 변환
[43] Phase 39-2: Carrier Pool + Virtual Thread 메트릭 수집             [██████████] 100%  ✅
     · java_virtual_thread_collector.go: jcmd VM.info ForkJoinPool 폴링
     · Thread.print 파싱으로 VT Active/Waiting/Mounted 카운트
     · JavaMetrics.VirtualThreads 필드 추가 + emitVirtualThreadItems() OTel Gauge
[44] Phase 39-3: Pinning 감지 + 알림                                   [██████████] 100%  ✅
     · virtual_thread_alert.go: 4개 알림 규칙 (pinned/min>10, p99>1000ms, submit_failed>5, carrier>90%)
     · ExtractPinnedStacks: Top-3 스택 트레이스 StorageBackend 저장
     · virtual_thread_api.go: Collection Server 6개 REST API + 데모 데이터
[45] Phase 39-4: Virtual Thread 가젯 UI (4개)                          [██████████] 100%  ✅
     · Gadget-1: Carrier Pool Gauge (parallelism/active/queued/utilization 색상 임계)
     · Gadget-2: VT Count 가젯 (active/waiting/mounted + 30분 히스토리 차트)
     · Gadget-3: Pinning 경고 가젯 (이벤트 수·P99·Top-3 메소드, 스택 슬라이드패널)
     · Gadget-4: Submit-Failed Bar Chart (30분 1분 버킷, CRITICAL 임계선)
     · JDK 버전 배지 + Virtual Thread 섹션 자동 표시/숨김
[46] Phase 39-5: Thread Dump JSON 파싱 (Virtual Thread 포함)           [██████████] 100%  ✅
     · java_thread_dump.go: jcmd Thread.dump_to_file -format=json 파싱
     · threadContainers 순회 → Platform/VirtualThread 분리, state 정규화
     · /infra/middleware/thread-dump 페이지: Virtual Threads 탭 (RUNNING/WAITING/BLOCKED 필터)
     · 클릭 시 스택 펼침, Pinning 스택 하이라이트

── 출시 전 Critical 기능 보완 (Phase 40) ✅ ─────────────────────────────────
[47] Phase 40-1: RUM (Real User Monitoring)                           [██████████] 100%  ✅
     · /rum 페이지: Core Web Vitals (LCP/FID/CLS/INP) 탭 + Page Performance 탭 + Geography 탭
     · Session Replay 테이블 (디바이스 배지, 브라우저, 국가)
     · 글로벌 PoP 5개 리전 레이턴시 측정 + Backend 4개 API
[48] Phase 40-2: SRE Golden Signals 대시보드                           [██████████] 100%  ✅
     · /golden-signals: Latency/Traffic/Errors/Saturation 2×2 시계열 차트
     · 서비스별 SLO 연동 + Error Budget + Burn Rate 경고 (>1.0 빨강)
     · 6개 서비스 데모 데이터 + Backend 3개 API
[49] Phase 40-3: Python 3.13 Free-Threaded 모니터링                   [██████████] 100%  ✅
     · /runtime/python: GIL 경합률 (3.12↓) + Free-Thread 활용률 (3.13t)
     · asyncio Task 큐 깊이 + GC Generation별 수집 시간 차트
     · 5개 에이전트 데모 (FT 2 + GIL 3) + Backend 2개 API
[50] Phase 40-4: .NET AOT / Native AOT 모니터링                       [██████████] 100%  ✅
     · /runtime/dotnet: ThreadPool Starvation 감지 + GC Suspension Time
     · AOT 제한사항 경고 패널 (reflection/trimming warnings)
     · 5개 에이전트 데모 (AOT 2 + JIT 3) + Backend 2개 API
[51] Phase 40-5: Go 1.24 Scheduler Latency 메트릭                     [██████████] 100%  ✅
     · /runtime/go: Scheduler Latency 히스토그램 (9개 버킷)
     · GC STW Pause 라인 차트 + Goroutine 스택 바 차트
     · 6개 에이전트 데모 + Backend 3개 API
[52] Phase 40-6: Database Monitoring (쿼리 수준)                      [██████████] 100%  ✅
     · /database: Instances/Slow Queries/Locks/Wait Events 4탭
     · PostgreSQL/MySQL 엔진 배지, 커넥션 게이지, SQL 모노스페이스 표시
     · 4 인스턴스 + 8 슬로우 쿼리 + 3 락 + 6 대기 이벤트 + Backend 5개 API

── 수작업 필요 (실제 인프라 환경, 코드 작업과 병렬/순차 진행) ───────────────
[10] Phase  7': E2E 통합 검증 (완료 — PASS 34/WARN 13/버그 7건 수정)  [██████████] 100%  ✅ 🔧
[11] Phase  8': Kubernetes 통합 배포                            [██████████] 100%  ✅ 🔧
     · Helm 차트: Frontend + Collection Server Deployment/Service/HPA/PVC
     · Ingress + TLS + NetworkPolicy + Secrets 템플릿
     · kind 4노드 클러스터 설정 + k8s-deploy.sh 자동화 스크립트
     · values-dev.yaml / values-prod.yaml AITOP 서비스 설정
     · nfpm DEB/RPM 패키지 + systemd 유닛 + Makefile 타겟
[12] Phase  9': SLO 튜닝 + 운영 안정화                        [██████████] 100%  ✅ 🔧
     · SLO 임계치 설정 (6개 SLO + burn rate alert + recording rules)
     · Tail Sampling 3계층 최적화 (비용 35% 절감)
     · 릴리스 승인서 + 릴리스 준비 점검 스크립트

═══════════════════════════════════════════════════════════════════════
  TO-DO — 실데이터 연동 (v1.1 — 하드코딩 demo-data.ts 제거)
═══════════════════════════════════════════════════════════════════════

⚠️  현재 상태: Frontend 32개 페이지가 demo-data.ts 하드코딩 데이터를 사용.
    실제 Prometheus/Jaeger/Agent 데이터가 UI에 표시되지 않음.
    Tempo(AGPL) → Jaeger(Apache-2.0) 교체 필요 (Phase 30 결정 미반영).

── Phase 41: 인프라 실연동 기반 구축 ─────────────────────────────────────────
[53] Phase 41-1: Tempo → Jaeger 교체 (AGPL-free 정합성)          [░░░░░░░░░░]   0%  📋
     · docker-compose.monitoring.yaml: Tempo 제거 → Jaeger all-in-one 교체
     · OTel Collector exporter: otlp/tempo → otlp/jaeger 변경
     · demo-site 문서 현행화 (DEMO_SETUP.md)
     · Helm values: tempo.enabled=false, jaeger 섹션 추가
[54] Phase 41-2: AITOP Agent 빌드 + 데모 서버 설치               [░░░░░░░░░░]   0%  📋
     · Agent 바이너리 빌드 (Windows amd64)
     · agent.yaml 설정 (server.endpoint → Collection Server)
     · OS Collector 동작 확인 (CPU/Memory/Disk/Network → Collection Server)
     · 데모 앱 프로세스 자동 감지 (Go/Python/Node.js PID)
     · Heartbeat → Fleet Console 에이전트 목록 표시 확인
[55] Phase 41-3: Collection Server 실데이터 프록시 구현          [░░░░░░░░░░]   0%  📋
     · Prometheus Query API 프록시 — /api/v1/prometheus/query
     · Jaeger Query API 프록시 — /api/v1/traces/search, /api/v1/traces/{traceId}
     · Agent Heartbeat 데이터 → /api/v1/fleet/agents 실데이터 반환
     · Agent Collect 데이터 → /api/v1/infra/hosts/{id}/metrics 실데이터 반환
     · 환경변수: PROMETHEUS_URL, JAEGER_URL 설정

── Phase 42: Frontend 실데이터 전환 (Tier 1 — 핵심 대시보드) ───────────────
[56] Phase 42-1: 홈 대시보드 (/) 실데이터 전환                   [░░░░░░░░░░]   0%  📋
     · demo-data.ts → api-client.ts fetch 전환
     · KPI 카드: 실시간 서비스 수, 에러율, P95 레이턴시 (Prometheus 쿼리)
     · 서비스 상태 테이블: Collection Server /fleet/agents 실데이터
     · 알림 카운트: Collection Server /alerts 실데이터
[57] Phase 42-2: 인프라 뷰 (/infra) 실데이터 전환               [░░░░░░░░░░]   0%  📋
     · 호스트 목록: Agent Heartbeat 데이터 (hostname, OS, CPU, Memory, Disk)
     · 호스트 상세: Agent Collect 데이터 (CPU 시계열, Memory 시계열)
     · GPU 뷰: Agent GPU Collector 데이터
     · 미들웨어 뷰: Agent 미들웨어 Collector 데이터
[58] Phase 42-3: 에이전트 Fleet 콘솔 (/agents) 실데이터 전환    [░░░░░░░░░░]   0%  📋
     · 에이전트 목록: Heartbeat 기반 실시간 상태 (online/offline)
     · 에이전트 상세: 수집 이력, 설정, 로그
     · 원격 CLI: 실제 Agent gRPC 연동
     · 그룹 관리: 실제 그룹 CRUD API 연동
[59] Phase 42-4: 트레이스 뷰 (/traces) 실데이터 전환            [░░░░░░░░░░]   0%  📋
     · XLog 산점도: Jaeger span 데이터 → 응답시간 scatter plot
     · 트레이스 상세: Jaeger traceId → waterfall timeline
     · 서비스 간 의존성: Jaeger dependency API

── Phase 43: Frontend 실데이터 전환 (Tier 2 — 모니터링 뷰) ───────────────
[60] Phase 43-1: 서비스 뷰 (/services) 실데이터 전환            [░░░░░░░░░░]   0%  📋
     · 서비스 목록: Prometheus service_name 기반 집계
     · 서비스 상세: Golden Signals (Prometheus 쿼리)
     · 엔드포인트 목록: Jaeger span 기반 집계
[61] Phase 43-2: 메트릭 탐색기 (/metrics) 실데이터 전환         [░░░░░░░░░░]   0%  📋
     · PromQL 직접 실행: Prometheus /api/v1/query_range 프록시
     · 메트릭 카탈로그: Prometheus /api/v1/label/__name__/values
     · 차트 렌더링: 실시간 Prometheus 시계열 데이터
[62] Phase 43-3: 로그 뷰 (/logs) 실데이터 전환                  [░░░░░░░░░░]   0%  📋
     · Collection Server 로그 수집 API (Agent → syslog/journald 전달)
     · 또는 Loki 대체재 연동 (VictoriaLogs 등 Apache-2.0)
[63] Phase 43-4: 알림 뷰 (/alerts) 실데이터 전환                [░░░░░░░░░░]   0%  📋
     · Prometheus Alertmanager API 프록시
     · 인시던트 목록: 실제 firing 알림 표시
     · 알림 정책: Prometheus Rule CRUD
[64] Phase 43-5: 토폴로지 (/topology) 실데이터 전환             [░░░░░░░░░░]   0%  📋
     · Agent netscanner 데이터 → D3 서비스맵
     · Jaeger dependency API → 서비스 간 화살표
     · 프로토콜 감지: Agent 수집 데이터

── Phase 44: Frontend 실데이터 전환 (Tier 3 — AI/고급 뷰) ───────────────
[65] Phase 44-1: AI 서비스 뷰 (/ai) 실데이터 전환               [░░░░░░░░░░]   0%  📋
     · LLM 메트릭: Prometheus aiservice_llm_* 쿼리
     · GPU 클러스터: Agent GPU Collector 데이터
     · RAG 파이프라인: Jaeger span 기반 단계별 분해
     · 가드레일: Prometheus aiservice_guardrail_* 쿼리
[66] Phase 44-2: SLO 뷰 (/slo) 실데이터 전환                    [░░░░░░░░░░]   0%  📋
     · SLO 목록: Collection Server SLO CRUD API
     · Error Budget: Prometheus recording rule 조회
     · Burn Rate: Prometheus slo:* 시계열
[67] Phase 44-3: 프로파일링 (/profiling) 실데이터 전환           [░░░░░░░░░░]   0%  📋
     · 프로파일 목록: StorageBackend 조회
     · FlameGraph: 실제 folded stack → SVG 렌더링
     · Trace 연결: Jaeger traceId 링크
[68] Phase 44-4: 진단 (/diagnostics) 실데이터 전환              [░░░░░░░░░░]   0%  📋
     · 진단 실행 목록: Evidence API 실데이터
     · 항목별 결과: StorageBackend ZIP → JSON 파싱
     · 교차 분석: IT↔AI 교차 데이터
[69] Phase 44-5: 나머지 페이지 실데이터 전환                    [░░░░░░░░░░]   0%  📋
     · /anomalies — 이상 탐지 (Prometheus 동적 임계)
     · /copilot — NL→PromQL 실제 Prometheus 쿼리 실행
     · /golden-signals — Prometheus Golden Signals 쿼리
     · /rum, /runtime/*, /database — Collection Server API 연동
     · /batch — 실제 배치 실행 이력 조회
     · /cloud, /pipelines, /business, /marketplace — 실데이터 또는 외부 API 연동

── Phase 45: 데모 사이트 OTel 계측 완성 ──────────────────────────────────
[70] Phase 45-1: Java OTel Agent 자동 연동                       [░░░░░░░░░░]   0%  📋
     · opentelemetry-javaagent.jar 다운로드 + -javaagent 옵션
     · OTEL_EXPORTER_OTLP_ENDPOINT 환경변수 설정
     · Spring Boot 데모 앱 + Spring Batch 트레이스 확인
[71] Phase 45-2: Python OTel 자동 계측                           [░░░░░░░░░░]   0%  📋
     · opentelemetry-instrument CLI 래퍼 사용 (코드 변경 없음)
     · FastAPI + psycopg + redis + httpx 자동 계측
     · RAG/Guardrail 서비스 트레이스 확인
[72] Phase 45-3: Go OTel SDK 통합                                [░░░░░░░░░░]   0%  📋
     · otelgin 미들웨어 추가 (Gin 자동 계측)
     · otelpgx 래퍼 (PostgreSQL 쿼리 트레이스)
     · otelredis 래퍼 (Redis 명령 트레이스)
[73] Phase 45-4: .NET OTel 계측                                  [░░░░░░░░░░]   0%  📋
     · OpenTelemetry.Instrumentation.AspNetCore (이미 csproj에 포함)
     · OTEL_EXPORTER_OTLP_ENDPOINT 환경변수 설정
     · EF Core + Redis + SignalR 트레이스 확인
[74] Phase 45-5: 데모 사이트 문서 + 실행 스크립트 완성          [░░░░░░░░░░]   0%  📋
     · DEMO_SETUP.md 업데이트: 전체 연동 절차 (Agent 설치 → OTel → AITOP)
     · run-demo.sh: 원클릭 전체 기동 스크립트
     · stop-demo.sh: 전체 종료 스크립트
     · verify-integration.sh: 연동 검증 스크립트 (Jaeger 트레이스 + Prometheus 메트릭 + Agent Fleet)

── Phase 46: 커스텀 대시보드 시스템 ──────────────────────────────────────────
[75] Phase 46-1: 대시보드 빌더 엔진                               [░░░░░░░░░░]   0%  📋
     · 드래그 앤 드롭 가젯 배치 (react-grid-layout 기반)
     · 가젯 추가/제거/리사이즈/이동 — 편집 모드 토글 (🔒보기 / ✏️편집)
     · 대시보드 레이아웃 저장/불러오기 (JSON → Collection Server 또는 LocalStorage)
     · 대시보드 CRUD: 생성 / 복제 / 이름 변경 / 삭제
     · 대시보드 목록 페이지 (/dashboards) — 내 대시보드 + 공유 대시보드 + 템플릿
[76] Phase 46-2: 가젯 라이브러리 (위젯 팔레트)                    [░░░░░░░░░░]   0%  📋
     · 가젯 타입 정의: line, area, bar, gauge, table, stat, heatmap, scatter, pie, text
     · 가젯 설정 패널 (사이드 드로어):
       - 데이터 소스 선택 (Prometheus PromQL / Collection Server API)
       - 시간 범위 (대시보드 전역 or 가젯 개별)
       - 새로고침 주기 (5s / 10s / 30s / 1m / off)
       - 임계치 색상 (warning/critical 값 + 색상)
     · 가젯 프리셋 템플릿: APM 기본, AI 서비스, 인프라, 배치, GPU 등 카테고리별
[77] Phase 46-3: 가젯 보기 모드 전환 (SUM / 개별 인스턴스)       [░░░░░░░░░░]   0%  📋
     · 가젯 우측 상단 아이콘: 📊 SUM(모아보기) ↔ 📈 개별(인스턴스별)
     · SUM 모드: 전체 인스턴스 합산/평균 — 단일 라인 차트
     · 개별 모드: 인스턴스별 라인 (색상 구분, 범례 표시)
     · 인스턴스 수가 10개 초과 시 개별 모드 자동 경고 + Top-N 필터
     · "금일 TPS", "금일 사용자 수"처럼 비교 전용 가젯은 SUM 모드 고정
[78] Phase 46-4: 대시보드 공유 + 내보내기                          [░░░░░░░░░░]   0%  📋
     · 대시보드 공유 URL 생성 (읽기 전용 공유 링크)
     · JSON 내보내기/가져오기 (Grafana 호환 구조)
     · 대시보드 템플릿 마켓플레이스 연동 (Phase 23 Marketplace)
     · PDF/PNG 스냅샷 내보내기

── Phase 47: APM 서비스 대시보드 가젯 (WhaTap 참조) ──────────────────────────
[79] Phase 47-1: TPS 가젯                                         [░░░░░░░░░░]   0%  📋
     · 실시간 TPS (초당 트랜잭션 건수) — 5초 간격 집계, 최근 10분 표시
     · ECharts 영역 차트 (area) — X축 시간, Y축 TPS
     · 모아보기(SUM): 전체 인스턴스 합산 TPS — 단일 파란 영역
     · 개별보기: 인스턴스별 라인 (색상 구분, 범례 클릭으로 토글)
     · 가젯 우측 상단 [📊] / [📈] 아이콘으로 모드 전환
     · Prometheus 쿼리: rate(http_server_request_duration_seconds_count[5s])
[80] Phase 47-2: 금일 TPS 가젯 (일간 비교)                        [░░░░░░░░░░]   0%  📋
     · 오늘 하루 TPS 시간대별 추이 — X축 00:00~24:00, Y축 TPS
     · 비교 날짜 선택 (기본: 어제) — 날짜 피커 아이콘
     · 비교 군: 연한 회색 배경 영역으로 표시 (오늘: 파란 실선 / 비교일: 회색 영역)
     · SUM 모드 고정 (개별 인스턴스 전환 불가 — 일간 비교 전용)
     · 마우스 호버 시 툴팁: "현재: 1,250 TPS / 어제 같은 시각: 980 TPS (+27.6%)"
[81] Phase 47-3: 금일 사용자 수 가젯 (일간 비교)                  [░░░░░░░░░░]   0%  📋
     · 오늘 하루 동시접속 사용자 수 시간대별 추이
     · 비교 날짜 선택 (기본: 어제) — 날짜 피커 아이콘
     · 비교 군: 연한 회색 배경 영역 (오늘: 녹색 실선 / 비교일: 회색 영역)
     · SUM 모드 고정
     · 사용자 카운팅: 5분 이내 트랜잭션 발생 IP 기반 (에이전트 설정으로 쿠키 가능)
     · 툴팁: "현재: 3,420명 / 어제: 2,890명 (+18.3%)"
[82] Phase 47-4: 평균 응답시간 가젯                                [░░░░░░░░░░]   0%  📋
     · 5초 간격 트랜잭션 평균 응답시간 (ms) — 최근 10분 표시
     · ECharts 라인 차트 — 임계치 참조선 (warning: 노란 점선, critical: 빨간 점선)
     · 모아보기(SUM): 전체 평균 — 단일 라인
     · 개별보기: 인스턴스별 라인 (느린 인스턴스 즉시 식별)
     · 튜닝 지표로서의 의미: 응답시간 추이 ↔ 배포/설정 변경 상관 분석
     · Prometheus: histogram_quantile(0.5, rate(http_server_request_duration_seconds_bucket[5s]))
[83] Phase 47-5: 액티브 트랜잭션 가젯                              [░░░░░░░░░░]   0%  📋
     · 현재 진행 중인 트랜잭션 수 — 5초 갱신
     · ECharts 영역 차트 + 현재 값 큰 숫자 표시
     · 모아보기: 전체 인스턴스 합산
     · 개별보기: 인스턴스별 스택
     · 급증 시 색상 변환 (정상: 파랑 → 경고: 노랑 → 위험: 빨강)
[84] Phase 47-6: 액티브 스테이터스 가젯                            [░░░░░░░░░░]   0%  📋
     · 액티브 트랜잭션을 상태별 카운트로 표시 — 5초 갱신
     · 상태 분류 (수평 스택 바 또는 도넛):
       - METHOD (메소드 수행 중) — 파랑
       - SQL (SQL 수행 중) — 주황
       - HTTPC (외부 API 호출 중) — 보라
       - DBC (Connection Pool 대기) — 빨강 ⚠️
       - SOCKET (TCP 소켓 연결 중) — 빨강 ⚠️
     · DBC 또는 SOCKET ≥ 1 지속 시 경고 표시 (깜빡임 + 툴팁 "Connection Pool 고갈 의심")
     · 모아보기: 전체 합산 / 개별보기: 인스턴스별 상태 테이블
[85] Phase 47-7: 액티브 트랜잭션 스피드 차트                       [░░░░░░░░░░]   0%  📋
     · 3단 수평 플로우 시각화:
       - 좌측: RPS (초당 요청 건수) — 들어오는 화살표 + 숫자
       - 중앙: 처리 중인 트랜잭션 (원형 게이지 또는 물탱크)
       - 우측: TPS (초당 처리 건수) — 나가는 화살표 + 숫자
     · 실시간 애니메이션 (파티클 또는 흐름 효과)
     · RPS > TPS 지속 시 중앙 게이지 상승 → 빨강 전환 (적체 감지)
     · 모아보기: 전체 합산 / 개별보기: 불가 (SUM 고정)
[86] Phase 47-8: 동시접속 사용자 가젯                              [░░░░░░░░░░]   0%  📋
     · 실시간 브라우저 사용자 수 — 5초마다 최근 5분 이내 IP 기반 카운팅
     · ECharts 영역 차트 — 최근 30분 추이 + 현재 값 큰 숫자 표시
     · 사용자 구분 방법: IP (기본) 또는 Cookie (에이전트 설정)
     · 모아보기: 전체 합산 / 개별보기: 인스턴스별 (부하 분산 확인)
     · Prometheus: count(count_over_time(http_server_request_duration_seconds_count{...}[5m]) > 0) by (client_ip)
```
...
│                                                                             │
│  Phase 17 ✅ ─► [10] Phase 7' ✅ ────────► [11] Phase 8' ──► [12] Phase 9' │
│                                                                             │
...
│                                                                             │
│  Phase 34 (Runtime Attach) ──┐                                             │
│  JDK 21+ 환경 ───────────────┴─► [42~46] Phase 39 (Virtual Thread 모니터링)│
│                                                                             │
│  ── v1.1 실데이터 연동 ────────────────────────────────────────────────────  │
│                                                                             │
│  Phase 30 (AGPL-free) ──► [53] Phase 41-1 (Tempo→Jaeger 교체)             │
│  Phase 15~16 (Agent) ──► [54] Phase 41-2 (Agent 빌드+설치)                │
│  Phase 17 (Backend) ────► [55] Phase 41-3 (Collection Server 실데이터 프록시)│
│                                  │                                         │
│         ┌────────────────────────┘                                         │
│         ▼                                                                  │
│  [56~59] Phase 42 (Tier1: 홈·인프라·Fleet·트레이스)                       │
│         │                                                                  │
│         ▼                                                                  │
│  [60~64] Phase 43 (Tier2: 서비스·메트릭·로그·알림·토폴로지)              │
│         │                                                                  │
│         ▼                                                                  │
│  [65~69] Phase 44 (Tier3: AI·SLO·프로파일링·진단·나머지)                 │
│                                                                             │
│  (병렬) ─► [70~74] Phase 45 (데모 사이트 OTel 계측 완성)                  │
│                                                                             │
│  ── 커스텀 대시보드 + APM 가젯 (Phase 42~44와 병렬 진행 가능) ──────────  │
│                                                                             │
│  Phase 14-1 (대시보드 빌더 기존) ──► [75~78] Phase 46 (커스텀 대시보드)   │
│                                              │                             │
│                                              ▼                             │
│                                     [79~86] Phase 47 (APM 가젯 8종)       │
│                                     · TPS / 금일TPS / 금일사용자 / 응답시간 │
│                                     · 액티브트랜잭션 / 스테이터스 / 스피드   │
│                                     · 동시접속 사용자                       │
└─────────────────────────────────────────────────────────────────────────────┘
```
...
| [10] | Phase 7' | ✅ 완료 — PASS 34, WARN 13, 버그 7건 수정. 2026-03-26 실행 완료. |
...
## [10] Phase 7': E2E 통합 검증 ✅

> **목표**: 전체 시스템 E2E 검증 — 매뉴얼 + AI 교차검증 체계
> **전제**: 모든 코드 Phase(1~38) 완료
...
> **완료일**: 2026-03-26 — PASS 34, WARN 13, 버그 7건 수정
...
**Session 50 (2026-03-26) E2E 최종 실행 결과:**

| 구분 | 수 | 비고 |
|------|---:|------|
| **PASS** | **34** | 핵심 시나리오 전체 통과 |
| **WARN** | **13** | 비기능(성능 임계치 근접·SKIP 항목) — 기능 결함 아님 |
| **버그 수정** | **7** | 실행 중 발견·즉시 수정 완료 |

| # | 작업 | 검증 항목 | AI 실행 | 수동 검증 | 상태 |
|---|------|----------|:------:|:--------:|:----:|
| 7'-1 | Docker 통합 테스트 | 11컨테이너 기동 + 파이프라인 21체크 | ✅ PASS | ✅ PASS | ✅ |
| 7'-2 | 부하 테스트 | Locust P95=43ms, 실패율 0% | ✅ PASS | ✅ PASS | ✅ |
| 7'-3 | Trace 연속성 | Layer 1~5 전파 검증 | ✅ PASS | ✅ PASS | ✅ |
| 7'-4 | 보안 감사 | A01~A10 OWASP 감사 | ✅ PASS | ✅ PASS | ✅ |
...
> **전제**: Phase 7' 교차검증 통과 ✅ (2026-03-26 완료)
> **완료일**: 2026-03-26 (Session 51)
> **참조**: [DOCS/TEST_GUIDE.md](DOCS/TEST_GUIDE.md) — 매뉴얼 Level 8 + AI-L4 교차검증

| # | 작업 | 상세 | 상태 |
|---|------|------|:----:|
| 8'-1 | Frontend Helm 서브차트 | `frontend-deployment.yaml`, `frontend-service.yaml`, `frontend-hpa.yaml` — Next.js standalone Deployment + Service + HPA | ✅ |
| 8'-2 | Collection Server Helm 서브차트 | `collection-server-deployment.yaml`, `service.yaml`, `hpa.yaml`, `pvc.yaml`, `serviceaccount.yaml` — JWT/S3/DB Secret 연동 | ✅ |
| 8'-3 | Helm Dry-Run + kind 클러스터 | `kind-config.yaml` (4노드: CP+Worker×3), `k8s-deploy.sh` (setup/build/deploy/dry-run/status/teardown), values-dev/prod 업데이트 | ✅ |
| 8'-4 | Ingress + TLS + NetworkPolicy | `ingress.yaml` (nginx + cert-manager + /api → CS, / → FE), `networkpolicy.yaml`, `secrets.yaml` (JWT, DB, S3) | ✅ |
| 8'-5 | DEB/RPM 패키지 | `nfpm.yaml`, systemd 유닛 2개 (agent, collection-server), pre/post install 스크립트, Makefile `pkg-deb`/`pkg-rpm` | ✅ |

**신규 파일 (20개):**
- `helm/aiservice-monitoring/templates/collection-server-deployment.yaml`
- `helm/aiservice-monitoring/templates/collection-server-service.yaml`
- `helm/aiservice-monitoring/templates/collection-server-hpa.yaml`
- `helm/aiservice-monitoring/templates/collection-server-pvc.yaml`
- `helm/aiservice-monitoring/templates/collection-server-serviceaccount.yaml`
- `helm/aiservice-monitoring/templates/frontend-deployment.yaml`
- `helm/aiservice-monitoring/templates/frontend-service.yaml`
- `helm/aiservice-monitoring/templates/frontend-hpa.yaml`
- `helm/aiservice-monitoring/templates/ingress.yaml`
- `helm/aiservice-monitoring/templates/secrets.yaml`
- `helm/aiservice-monitoring/templates/networkpolicy.yaml`
- `infra/kubernetes/kind-config.yaml`
- `scripts/k8s-deploy.sh`
- `agent/nfpm.yaml`
- `agent/packaging/aitop-agent.service`
- `agent/packaging/aitop-collection-server.service`
- `agent/packaging/preinstall.sh`
- `agent/packaging/postinstall.sh`
- `agent/packaging/preremove.sh`

**수정된 파일:**
- `helm/aiservice-monitoring/values.yaml` — collectionServer, frontend, ingress, networkPolicy 섹션 추가
- `helm/aiservice-monitoring/values-dev.yaml` — dev 환경 AITOP 서비스 + Ingress 설정
- `helm/aiservice-monitoring/values-prod.yaml` — prod HA 구성 + TLS + NetworkPolicy
- `helm/aiservice-monitoring/templates/NOTES.txt` — AITOP 서비스 접속 정보 추가
- `agent/Makefile` — k8s-*, pkg-* 타겟 추가

**배포 명령:**
```bash
# 전체 자동화 (kind 생성 → 빌드 → 배포)
make -C agent k8s-all

# 개별 단계
make -C agent k8s-setup      # kind 클러스터 생성
make -C agent k8s-build      # Docker 이미지 빌드
make -C agent k8s-deploy     # Helm 배포 (dev)
make -C agent k8s-dry-run    # 검증만

# DEB/RPM 패키지
make -C agent pkg-all
```
...
> **목표**: SLO 임계치 설정 + Tail Sampling 비용 최적화 + 릴리스 승인
> **전제**: Phase 8' K8s 배포 설정 완료
> **완료일**: 2026-03-26 (Session 51)
> **참조**: [DOCS/TEST_GUIDE.md](DOCS/TEST_GUIDE.md) — 매뉴얼 Level 9 + AI-L5 교차검증

| # | 작업 | 상세 | 상태 |
|---|------|------|:----:|
| 9'-1 | SLO 임계치 튜닝 | `slo-thresholds.yaml` — LLM/GPU/가드레일/외부API/벡터DB/인프라 전 항목 baseline/warning/critical 설정. Prometheus SLO Burn Rate Alert 6개 + Recording Rule 7개 추가. `slo-tuning.sh` 자동 점검 스크립트 | ✅ |
| 9'-2 | Tail Sampling 최적화 | `otelcol-gateway-optimized.yaml` — 3계층 정책 (Always 100%→Conditional 10~50%→Probabilistic 3%). 비용 35% 절감 ($23.50→$15.20/월). `sampling-optimizer.py` 비용 분석기 | ✅ |
| 9'-3 | 교차검증 최종 보고서 | `release-readiness.sh` — 7 카테고리 자동 점검 (Build/Tests/Security/SLO/Docs/Infra/Monitoring). `RELEASE_APPROVAL.md` — v1.0.0 릴리스 승인서 (테스트 431건 PASS, OWASP 10항목, 6 SLO) | ✅ |

**신규 파일:**
- `infra/slo/slo-thresholds.yaml` — SLO 임계치 중앙 설정 (6개 카테고리, 15개 메트릭)
- `scripts/slo-tuning.sh` — SLO 임계치 점검/보고서 생성 도구
- `collector/config/otelcol-gateway-optimized.yaml` — 3계층 최적화 Tail Sampling 설정
- `scripts/sampling-optimizer.py` — Tail Sampling 비용 최적화 분석기
- `scripts/release-readiness.sh` — 릴리스 준비 상태 자동 점검 (7 카테고리)
- `reports/RELEASE_APPROVAL.md` — v1.0.0 릴리스 승인서

**수정된 파일:**
- `helm/aiservice-monitoring/templates/prometheus-rules.yaml` — SLO Burn Rate Alert 6개 + Recording Rule 7개 추가

---

## [15~18] Phase 31: 에이전트 일원화 (ADR-011) 📋
...
> **카탈로그**: [DOCS/DIAGNOSTIC_ITEMS_CATALOG.md](DOCS/DIAGNOSTIC_ITEMS_CATALOG.md) — 항목별 출처(📊/🔍/📊🔍) 및 수집 방식(🔧/📜/🖐️) 분류 기준

| # | 작업 | 상세 | 예상 기간 | 상태 |
|---|------|------|---------|:----:|
| **31-1** | **내장(🔧) 진단 항목 Go 구현** | `--mode=diagnose` CLI + 카탈로그 🔧 분류 항목 직접 구현 | 3~4주 | ✅ |
| 31-1a | Evidence Collector 인터페이스 | `agent/internal/collector/evidence/` 패키지 — `EvidenceCollector` 인터페이스 + Registry 정의 | | ✅ |
| 31-1b | ConfigEvidence 플러그인 | 설정 파일 수집 — ITEM0009·0012·0041·0045·0051·0052 (nginx.conf, server.xml, my.cnf 등) | | ✅ |
| 31-1c | LogEvidence 플러그인 | 로그 패턴 분석 — ITEM0008·0011·0016·0026·0027·0036·0055 (GC/에러/슬로우 쿼리/접근 로그) | | ✅ |
| 31-1d | EOSEvidence 플러그인 | 내장 lifecycle DB 버전 체크 — ITEM0068 (Java/Node/Python/Go/MySQL/PostgreSQL/Redis/Nginx/Apache) | | ✅ |
| 31-1e | Evidence ZIP 생성 + 업로드 | 구조화 JSON → ZIP (`ZipBuilder`) → HTTPS POST (`EvidenceUploader`) | | ✅ |
| 31-1f | 기존 Collector → Evidence 변환 어댑터 | `CollectorAdapter` — 12종 Collector 출력을 Evidence JSON으로 변환 | | ✅ |
| 31-1g | 🔧 내장 진단 항목 구현 | ITEM0013·0015·0037·0040·0044·0063·0064·0066·0070 — `/proc`/sysfs 기반 스냅샷 (`BuiltinItemsCollector`) | | ✅ |
| **31-2** | **스크립트(📜) 실행 프레임워크** | Script Executor + 진단 스케줄러 + Backend 연동 | 2~3주 | ✅ |
| 31-2a | Script Executor 구현 | `agent/internal/script/executor.go` — `.sh`/`.ps1`/`.py` 실행 + 타임아웃 + 출력 캡처 | | ✅ |
| 31-2b | 진단 스케줄러 | `--mode=diagnose` + `diagnose.Runner` cron 스케줄링. `ModeDiagnose` 추가 | | ✅ |
| 31-2c | 📜 항목 스크립트 매핑 테이블 | `agent.yaml`의 `diagnostic.scripts` 섹션 + `DiagnosticConfig` 구조체 추가 | | ✅ |
| 31-2d | Evidence 수신 API | `POST /api/v1/evidence/upload` + `GET /api/v1/evidence` (collection-server) | | ✅ |
| 31-2e | Backend 릴레이 + 자동 진단 트리거 | `transport.EvidenceRelayClient` — Relay + TriggerDiagnosis | | ✅ |
| 31-2f | Fleet에 진단 상태 표시 | `models.DiagnosticStatus` + Heartbeat.Diagnostic 필드 추가 | | ✅ |
| **31-3** | **수동(🖐️) 트리거 방식 + 고급 진단** | UI 트리거 API + 실행 권한 제어 + 고급 진단 플러그인 | 3~4주 | ✅ |
| 31-3a | 🖐️ 항목 UI 트리거 API | `POST /api/v1/agents/{id}/diagnose` — role 검증 + audit 로그 + EventBus 발행 | | ✅ |
| 31-3b | 실행 권한 및 감사 로그 | `diagnose.AccessController` + `AuditLog` (append-only NDJSON) | | ✅ |
| 31-3c | SecurityEvidence 플러그인 | SELinux/AppArmor·오픈 포트·패스워드 정책·sudoers — ITEM0056·0057·0065·0067 | | ✅ |
| 31-3d | APMEvidence 플러그인 | 6종 APM SaaS 설정 탐지 — ITEM0054 (WhaTap/NR/DD/DT/Scouter/OTel) | | ✅ |
| 31-3e | CrossAnalysis 플러그인 | IT↔AI 교차 분석 스냅샷 (`ModeManual`) — ITEM0222·0224·0226 | | ✅ |
| 31-3f | `--mode=collect-only` | 기존 구현 유지 (에어갭 1회 수집→Push→종료) | | ✅ |
| 31-3g | `--mode=full` | 상시 모니터링 + 진단 스케줄 자동 등록 (`buildDiagnoseRunner`) | | ✅ |
| **31-4** | **Java Agent EOL** | 기능 동등성 검증 + 마이그레이션 + 병행 운영 | 4~8주 | ✅ |
| 31-4a | 기능 동등성 테스트 | `equivalence_test.go` — 31개 카탈로그 항목 Go 커버 확인, 5개 스크립트 항목 추적 | | ✅ |
| 31-4b | 마이그레이션 가이드 | `DOCS/JAVA_AGENT_MIGRATION_GUIDE.md` — Step-by-step 전환 가이드, 설정 매핑표, FAQ | | ✅ |
| 31-4c | 병행 운영 (3개월) | `DOCS/JAVA_AGENT_PARALLEL_OPERATION.md` — 고객 유형별 정책, 동시 실행 가이드라인 | | ✅ |
| 31-4d | Java Agent EOL 선언 | `DOCS/JAVA_AGENT_EOL_TIMELINE.md` — EOL 2026-09-30, 공지 템플릿, 릴리스 계획 | | ✅ |

**성공 지표**:
- 설치 크기 < 30MB (단일 바이너리)
- 상주 메모리 < 60MB (full 모드)
- 진단 항목 커버리지 91건 전체 (IT 55 + AI 31 + AI Serving 5건 신규)
- 상시 모니터링 메트릭 35종 전체 지속 수집 (OS 6 + GPU 4 + WEB 2 + WAS/미들웨어 6 + Cache 11 + MQ 6)
...
## [41] Phase 38: 배치 대시보드 뷰 + 알림 ✅

> **Session 49** | 2026-03-25 완료
> **의존성**: Phase 36 (완료), Phase 37 (완료), Phase 35 플레임그래프 UI (완료)

### 38-1~4. 배치 대시보드 UI (5개 페이지) ✅

| 파일 | 설명 |
|------|------|
| `frontend/src/app/batch/page.tsx` | 잡 목록 — KPI 카드, 필터, 상태 배지, 타임라인 scatter |
| `frontend/src/app/batch/[name]/page.tsx` | 잡 상세 — 실행 이력 바 차트, 실행 테이블 |
| `frontend/src/app/batch/executions/[id]/page.tsx` | 실행 상세 — 5탭 (Overview/SQL/Methods/Flamegraph/Runtime) |
| `frontend/src/app/batch/xlog/page.tsx` | XLog 산점도 — duration × time, SLA 임계선, 클릭→상세 |
| `frontend/src/app/batch/alerts/page.tsx` | 알림 규칙 CRUD + 이력 탭 |

### 38-5. 플레임그래프 연동 ✅

- 실행 상세 Flamegraph 탭에서 Phase 35 SystemFlamegraph 컴포넌트 재사용
- 배치 실행 시간 범위 자동 선택
...
| 파일 | 설명 |
|------|------|
| `agent/cmd/collection-server/batch_alert_api.go` | 6개 API: rules CRUD, history, evaluate |

데모 규칙: Order Settlement SLA, Batch Failure Alert, ETL Slow Warning, High CPU Usage

### 통합 ✅

| 파일 | 설명 |
|------|------|
| `frontend/src/components/layout/sidebar.tsx` | 배치 모니터링 네비게이션 추가 (Timer 아이콘) |
| `frontend/src/lib/i18n.ts` | nav.batch 번역 (ko/en/ja) |
| `frontend/src/types/monitoring.ts` | BatchJob, BatchExecution 등 8개 타입 |
| `frontend/src/lib/demo-data.ts` | 8개 데모 데이터 함수 (잡/실행/SQL/메소드/XLog/알림) |
...
> **Session 49** | 2026-03-25 완료
> **의존성**: Phase 36 (완료), Phase 34 Runtime Attach (완료), Phase 35 perf/eBPF (완료)

### 37-1. Java 배치 프로파일링 ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/collector/batch/profiler/java_batch_profiler.go` | async-profiler JDBC SQL Top-N + 메소드 Top-N + jcmd JVM 메트릭 |

### 37-2. Python 배치 프로파일링 ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/collector/batch/profiler/python_batch_profiler.go` | py-spy folded stack → 함수 Top-N + py-spy dump 스레드 덤프 |

### 37-3. .NET 배치 프로파일링 ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/collector/batch/profiler/dotnet_batch_profiler.go` | EventPipe EF Core 쿼리 + dotnet-counters GC/Gen0/1/2/ThreadPool |

### 37-4. Go + perf/eBPF 플레임그래프 연동 ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/collector/batch/profiler/go_batch_profiler.go` | pprof HTTP cpu/heap/goroutine 자동 수집 |
| `agent/internal/collector/batch/profiler/flamegraph_integration.go` | Phase 35 perf/eBPF PID 타겟 on-CPU/off-CPU 플레임그래프 |
| `agent/internal/collector/batch/profiler/orchestrator.go` | 언어별 프로파일러 동시 실행 오케스트레이션 |

### Collection Server API ✅

| 파일 | 설명 |
|------|------|
| `agent/cmd/collection-server/batch_profiling_api.go` | 8개 API: profile, sql, methods, flamegraph, gc, stack, trigger, upload |

데모: Java SQL (INSERT/UPDATE/SELECT 쿼리 타이밍), Spring Batch 메소드 핫스팟, Python pandas/numpy/torch Top-N
...
> **Session 49** | 2026-03-25 완료
> **의존성**: Phase 34 Runtime Attach (완료), Phase 35 perf/eBPF (완료)

### 36-1. 배치 프로세스 감지기 ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/collector/batch/detector.go` | 4단계 자동 감지: cron/systemd 자식, 프레임워크 패턴, 수동 태그 |

감지 규칙: cron ppid → systemd timer → Spring Batch/Quartz/Airflow/Celery 커맨드 패턴 → agent.yaml 수동 태그

### 36-2. 프로세스 레이어 Collector ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/collector/batch/process_collector.go` | /proc/{pid}/stat, status, io 폴링 → CPU%/RSS/VmPeak/IO |

### 36-3~4. 생명주기 관리 + 데이터 모델 ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/collector/batch/lifecycle.go` | 상태 머신 (DETECTED→RUNNING→COMPLETED/FAILED), 메트릭 집계 |
| `agent/internal/collector/batch/batch_collector.go` | models.Collector (ID: "batch"), schemas: execution.v1, metrics.v1 |
| `agent/internal/collector/batch/register.go` | Registry 등록 헬퍼 |

### 36-5. 프레임워크 연동 ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/collector/batch/framework/spring_batch.go` | BATCH_JOB_EXECUTION 테이블 폴링 |
| `agent/internal/collector/batch/framework/airflow.go` | REST API `/api/v1/dags/{dag_id}/dagRuns` 수집 |

### Collection Server API ✅

| 파일 | 설명 |
|------|------|
| `agent/cmd/collection-server/batch_api.go` | 7개 API: executions CRUD, jobs, running, stats |

API: `GET /batch/executions`, `GET /batch/executions/{id}`, `POST /batch/executions`, `GET /batch/jobs`, `GET /batch/jobs/{name}`, `GET /batch/running`, `GET /batch/stats`

데모 데이터: 7개 배치 잡 × 24개 실행 이력 (daily-order-settlement, customer-email-campaign, data-warehouse-etl 등)
...
### 33-1. Plugin Manager + File Watcher (에이전트 측) ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/plugin/manifest.go` | PluginManifest 타입 + YAML 파서 + SHA-256 체크섬 검증 |
| `agent/internal/plugin/manager.go` | PluginManager 코어: Load/Unload/Execute/Rollback/Install(ZIP) |
| `agent/internal/plugin/watcher.go` | 폴링 기반 파일 감시 (5초 주기, fsnotify 의존성 없음) |
| `agent/internal/plugin/executor.go` | Script Executor (.sh/.ps1/.py/.bat) + 타임아웃 + 4MiB 출력 제한 |
| `agent/internal/plugin/collector_adapter.go` | PluginCollector — models.Collector 인터페이스 어댑터 |

### 33-2. Plugin Registry + Deploy API (서버 측) ✅

| 파일 | 설명 |
|------|------|
| `agent/cmd/collection-server/plugin_api.go` | 10개 REST API (§9.5.9), in-memory 레지스트리 |
| `agent/pkg/storage/keys.go` | PluginKey 추가 (plugins/ 키 패턴) |
| `agent/cmd/collection-server/main.go` | registerPluginRoutes 호출 추가 |

API: `GET/POST /fleet/plugins`, `GET/DELETE /fleet/plugins/{name}`, `POST deploy/rollback/disable`, `GET status/history/download`

### 33-3. 배포 전략 + 자동 롤백 ✅

| 전략 | 구현 |
|------|------|
| 즉시 (immediate) | 전체 대상 에이전트에 즉시 배포 명령 |
| 단계 (staged) | canary → 10% → 50% → 100% 자동 확장 (30초 배경 ticker) |
| 예약 (scheduled) | 지정 시각 도달 시 즉시 배포로 전환 (30초 체크) |
| 자동 롤백 | 실패율 >50% 감지 → 자동 롤백 명령 발송 |
| 수동 롤백 | `.rollback/` 디렉토리 swap (1개 이전 버전 보관) |

### 33-4. Fleet Console Plugin UI ✅

| 파일 | 설명 |
|------|------|
| `frontend/src/app/agents/plugins/page.tsx` | 플러그인 목록 (KPI 카드, 레지스트리 테이블, 배포 이력) |
| `frontend/src/app/agents/plugins/[name]/page.tsx` | 플러그인 상세 (에이전트별 설치 상태, Deploy/Rollback) |
| `frontend/src/components/monitoring/deploy-strategy-modal.tsx` | 배포 전략 모달 (즉시/단계/예약, 대상 선택) |
| `frontend/src/types/monitoring.ts` | PluginRegistryItem, PluginDeployHistory 등 6개 타입 |
| `frontend/src/lib/demo-data.ts` | 6개 플러그인 + 8개 배포 이력 + 에이전트별 상태 데모 |

---

## [35~38] Phase 35: perf/eBPF 시스템 프로파일링 + 플레임그래프 ✅

> **Session 49** | 2026-03-25 완료
> **의존성**: Phase 34 Runtime Attach (완료), Phase 27 StorageBackend (완료)

### 35-1. perf/eBPF Collector 모듈 (에이전트 측) ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/collector/perfebpf/perfebpf_collector.go` | 메인 Collector (ID: `perf-ebpf`), on-CPU/off-CPU/memory 오케스트레이션 |
| `agent/internal/collector/perfebpf/capability.go` | Linux capability 감지 (CAP_BPF, CAP_PERFMON, CAP_SYS_ADMIN, CAP_SYS_PTRACE) |
| `agent/internal/collector/perfebpf/oncpu_profiler.go` | perf record + perf script → folded stack 변환 |
| `agent/internal/collector/perfebpf/offcpu_profiler.go` | sched:sched_switch 트레이스포인트 / bpftrace 폴백 |
| `agent/internal/collector/perfebpf/memory_profiler.go` | mmap 트레이스포인트 / bpftrace malloc uprobe / smaps 폴백 |
| `agent/internal/collector/perfebpf/folded_stack.go` | Merge/Compress/Decompress/Validate/FilterByPID 유틸리티 |
| `agent/internal/collector/perfebpf/errors.go` | perf/eBPF 전용 에러 코드 6종 |
| `agent/internal/collector/perfebpf/register.go` | Registry 등록 헬퍼 |
| `agent/internal/collector/it/register.go` | perfebpf.New() 등록 추가 |
| `agent/configs/agent.yaml` | profiling.perf_ebpf 설정 섹션 추가 |

### 35-2. 언어별 JIT 심볼 해석 ✅

| 파일 | 설명 |
|------|------|
| `agent/internal/collector/perfebpf/symbol/resolver.go` | SymbolResolver 인터페이스 + ResolveSymbols + loadPerfMaps |
| `agent/internal/collector/perfebpf/symbol/util.go` | 공통 헬퍼 (readFileIfExists, perfMapPath, detectProcessRuntime) |
| `agent/internal/collector/perfebpf/symbol/java_resolver.go` | perf-map-agent / JVM Attach API 연계 |
| `agent/internal/collector/perfebpf/symbol/python_resolver.go` | py-spy dump / CPython 3.12+ frame pointer |
| `agent/internal/collector/perfebpf/symbol/nodejs_resolver.go` | V8 --perf-basic-prof / SIGUSR2 트리거 |
| `agent/internal/collector/perfebpf/symbol/go_resolver.go` | DWARF 심볼 (네이티브, -trimpath 감지) |
| `agent/internal/collector/perfebpf/symbol/dotnet_resolver.go` | crossgen2 PerfMap / DOTNET_EnablePerfMap |

### 35-3. Collection Server 플레임그래프 생성 엔진 ✅

| 파일 | 설명 |
|------|------|
| `agent/cmd/collection-server/flamegraph_api.go` | 5개 API: flamegraph, stacks, trigger, system/profiles, system/upload |
| `agent/cmd/collection-server/flamegraph_svg.go` | Brendan Gregg SVG 생성기 (3색상 + 인터랙티브 JS: 줌/검색/툴팁) |
| `agent/cmd/collection-server/diff_flamegraph.go` | 차등 플레임그래프 SVG (빨강=증가, 파랑=감소) |
| `agent/pkg/storage/keys.go` | PerfProfileKey 추가 (perf-profiles/ 키 패턴) |
| `agent/cmd/collection-server/main.go` | registerFlamegraphRoutes 호출 추가 |

### 35-4. 인터랙티브 플레임그래프 UI ✅

| 파일 | 설명 |
|------|------|
| `frontend/src/app/profiling/system/page.tsx` | 시스템 프로파일링 페이지 (KPI 카드, 에이전트 선택, 타입 탭, 프로필 목록) |
| `frontend/src/components/charts/system-flamegraph.tsx` | 향상된 플레임그래프 (줌/검색/우클릭 메뉴, 타입별 색상, 미니맵) |
| `frontend/src/types/monitoring.ts` | SystemProfile, SystemFlamegraphData 타입 추가 |
| `frontend/src/lib/demo-data.ts` | getSystemProfilingProfiles, getSystemFlamegraphData, getSystemFlamegraphDiffData |
...
| `DOCS/DIAGNOSTIC_ITEMS_CATALOG.md` | ✅ v1.0.0 | 86개 진단 항목 카탈로그 (IT 55 + AI 31) — 출처·수집방식 분류 기준 |
| `DOCS/JAVA_AGENT_EOL_TIMELINE.md` | ✅ v1.0.0 | Java Agent EOL 타임라인 — 마이그레이션 단계·일정 (Phase 31-4) |
| `DOCS/JAVA_AGENT_MIGRATION_GUIDE.md` | ✅ v1.0.0 | Java Agent → AITOP Go Agent 마이그레이션 가이드 (Phase 31-4) |
| `DOCS/JAVA_AGENT_PARALLEL_OPERATION.md` | ✅ v1.0.0 | Java Agent 병행운영 가이드 — 전환 기간 데이터 정합성 보장 (Phase 31-4) |
...
## [42~46] Phase 39: JDK 21 Virtual Thread 모니터링 ✅

> **목표**: JDK 21+ 환경에서 Virtual Thread(Project Loom) 수집·가시화를 상용 APM 수준으로 구현. Pinning 감지·Carrier Pool 포화·스케줄링 지연을 실시간 알림으로 연결.
> **완료일**: 2026-03-26
> **의존성**: Phase 34 Runtime Attach (Java Attach API) ✅
> **설계 문서**: [DOCS/JAVA_DOTNET_SDK_DESIGN.md §10](DOCS/JAVA_DOTNET_SDK_DESIGN.md) | [DOCS/UI_DESIGN.md §16](DOCS/UI_DESIGN.md) | [DOCS/METRICS_DESIGN.md §14](DOCS/METRICS_DESIGN.md)

### 39-1. JFR Virtual Thread 이벤트 구독 (Agent Attach 확장)

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 39-1-1 | JDK 버전 자동 감지 | `VirtualThreadDetector`: Attach 시 `java.version` 조회 → 21+ 이면 Virtual Thread 모듈 활성화 | ✅ |
| 39-1-2 | JFR 이벤트 구독 설정 | `aitop-attach-helper.jar` 확장 — `jdk.VirtualThreadPinned` / `jdk.VirtualThreadSubmitFailed` / `jdk.VirtualThreadStart` / `jdk.VirtualThreadEnd` / `jdk.VirtualThreadPark` / `jdk.VirtualThreadUnpark` | ✅ |
| 39-1-3 | JFR 이벤트 → OTel 변환 | JFR 이벤트 필드를 OTel Metric/Event로 변환 후 Collection Server 전송 | ✅ |

### 39-2. Carrier Pool + Virtual Thread 메트릭 수집

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 39-2-1 | JMX Carrier Pool 폴링 | `ForkJoinPool` 내부 스케줄러 접근 — `parallelism` / `activeCount` / `queuedTaskCount` 10초 주기 수집 | ✅ |
| 39-2-2 | Virtual Thread 카운트 집계 | `ThreadMXBean` + JFR Start/End 이벤트 기반 활성/대기 수 추정 | ✅ |
| 39-2-3 | OTel Gauge 내보내기 | `jvm.virtual_thread.count`, `jvm.carrier_pool.*` 5개 메트릭 VictoriaMetrics 저장 | ✅ |

### 39-3. Pinning 감지 + 알림

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 39-3-1 | Pinning 이벤트 파싱 | `jdk.VirtualThreadPinned` 이벤트에서 `duration` + `stackTrace` 추출 | ✅ |
| 39-3-2 | Pinning 메트릭 생성 | `jvm.virtual_thread.pinned.count` (Counter) + `jvm.virtual_thread.pinned.duration_ms` (Histogram) | ✅ |
| 39-3-3 | 알림 규칙 추가 | Pinning rate/1m > 10 → WARNING, p99 > 1000ms → CRITICAL, submit_failed rate/1m > 5 → CRITICAL | ✅ |
| 39-3-4 | Pinning 스택 저장 | 상위 Pinning 스택 트레이스 StorageBackend 저장 (슬라이드패널 드릴다운용) | ✅ |

### 39-4. Virtual Thread 가젯 UI (4개)

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 39-4-1 | Carrier Pool 게이지 | ForkJoinPool active/parallelism 비율 — 색상 임계 (< 70%: 녹, 70~90%: 노, > 90%: 빨) | ✅ |
| 39-4-2 | Virtual Thread 카운트 가젯 | 활성/대기 수 + 최근 5분 생성 수 실시간 SSE 갱신 | ✅ |
| 39-4-3 | Pinning 경고 가젯 | 이벤트 수·p99 지속시간·Top-3 원인 메소드 표시. 클릭 시 스택 슬라이드패널 | ✅ |
| 39-4-4 | 스케줄링 지연 차트 | `jvm.virtual_thread.submit_failed` 1분 버킷 Bar chart (최근 30분) | ✅ |
| 39-4-5 | JDK 버전 배지 | 미들웨어 헤더에 JDK 버전 배지 + Virtual Thread 섹션 자동 표시/숨김 | ✅ |

### 39-5. Thread Dump JSON 파싱 (Virtual Thread 포함)

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 39-5-1 | jcmd 실행 | `jcmd <pid> Thread.dump_to_file -format=json /tmp/aitop-td-{pid}.json` 트리거 (Go Exec) | ✅ |
| 39-5-2 | JSON 파싱 | `threadContainers` 순회 — VirtualThreads 컨테이너 분리, 스레드별 state/stack 추출 | ✅ |
| 39-5-3 | Collection Server 저장 | Thread Dump JSON → StorageBackend (키: `thread-dumps/{agentId}/{timestamp}.json`) | ✅ |
| 39-5-4 | UI 표시 | 기존 Thread Dump 뷰에 "Virtual Threads" 탭 추가 — WAITING/RUNNING/BLOCKED 필터 | ✅ |

**신규 파일:**
- `agent/internal/attach/java_virtual_thread.go` — VirtualThreadDetector + JFR 설정 XML + OTel 변환
- `agent/internal/attach/java_thread_dump.go` — jcmd JSON 파싱 + ParseThreadDumpJSON
- `agent/internal/collector/middleware/java_virtual_thread_collector.go` — Carrier Pool + VT 메트릭 수집
- `agent/internal/collector/middleware/virtual_thread_alert.go` — Pinning 알림 규칙 4개
- `agent/cmd/collection-server/virtual_thread_api.go` — Collection Server 6개 REST API
- `frontend/src/app/infra/middleware/thread-dump/page.tsx` — Thread Dump 뷰어 (Virtual Threads 탭)

---

## [47~52] Phase 40: 출시 전 Critical 기능 보완 ✅

> **목표**: 상용 출시 전 부족한 모니터링 기능 6종 보완
> **완료일**: 2026-03-26 (Session 51)
...
| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 40-1-1 | RUM 페이지 | `/rum` — Core Web Vitals 탭(LCP/FID/CLS/INP 분포 차트), Page Performance 탭, Geography 탭 | ✅ |
| 40-1-2 | Session Replay 테이블 | 디바이스 배지(Desktop/Mobile/Tablet), 브라우저, 국가, 에러 수 | ✅ |
| 40-1-3 | 글로벌 PoP 메트릭 | 5개 리전 (Seoul/Tokyo/Singapore/US-West/Frankfurt) 레이턴시 | ✅ |
| 40-1-4 | Backend API | `GET /rum/sessions, /rum/pages, /rum/geo, /rum/stats` (4개 엔드포인트) | ✅ |

### 40-2. SRE Golden Signals 대시보드 ✅

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 40-2-1 | Golden Signals 페이지 | `/golden-signals` — 4 KPI + 2×2 시계열 차트 (Latency/Traffic/Errors/Saturation) | ✅ |
| 40-2-2 | 서비스 테이블 | SLO Target/Current, Error Budget, Burn Rate (>1.0 빨강, >0.5 노랑) | ✅ |
| 40-2-3 | Backend API | `GET /golden-signals/services, /timeseries, /stats` (3개 엔드포인트) | ✅ |

### 40-3. Python 3.13 Free-Threaded 모니터링 ✅

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 40-3-1 | Python Runtime 페이지 | `/runtime/python` — GIL/FT 모드 배지, 버전 필터, 에이전트 테이블 | ✅ |
| 40-3-2 | GC 프로파일링 차트 | Gen0/Gen1/Gen2 수집 시간 ECharts 스택 바 차트 | ✅ |
| 40-3-3 | Asyncio 차트 | Pending tasks 라인 차트 (12 데이터 포인트) | ✅ |
| 40-3-4 | Backend API | `GET /runtime/python, /runtime/python/stats` (2개 엔드포인트) | ✅ |

### 40-4. .NET AOT 모니터링 ✅

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 40-4-1 | .NET Runtime 페이지 | `/runtime/dotnet` — Native AOT/JIT 배지, 모드 필터, 에이전트 테이블 | ✅ |
| 40-4-2 | ThreadPool 차트 | Threads + Queue Length 라인 차트 | ✅ |
| 40-4-3 | GC 차트 | Gen0/Gen1/Gen2 수집 횟수 바 차트 | ✅ |
| 40-4-4 | AOT 경고 패널 | Reflection/Trimming 제한사항 경고 목록 | ✅ |
| 40-4-5 | Backend API | `GET /runtime/dotnet, /runtime/dotnet/stats` (2개 엔드포인트) | ✅ |

### 40-5. Go 1.24 Scheduler Latency ✅

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 40-5-1 | Go Runtime 페이지 | `/runtime/go` — 버전 필터, 에이전트 테이블 (P50/P95/P99 μs) | ✅ |
| 40-5-2 | Scheduler Histogram | 9개 버킷 (1μs~100ms+) ECharts 바 차트 | ✅ |
| 40-5-3 | GC STW 차트 | STW Pause 라인 차트 (12 데이터 포인트) | ✅ |
| 40-5-4 | Goroutine 차트 | Runnable vs Waiting 스택 바 차트 | ✅ |
| 40-5-5 | Backend API | `GET /runtime/go, /runtime/go/histogram, /runtime/go/stats` (3개 엔드포인트) | ✅ |

### 40-6. Database Monitoring ✅

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 40-6-1 | Database 페이지 | `/database` — 4탭 (Instances/Slow Queries/Locks/Wait Events) | ✅ |
| 40-6-2 | Instances 탭 | PostgreSQL/MySQL 엔진 배지, 커넥션 게이지, QPS, Cache Hit Ratio | ✅ |
| 40-6-3 | Slow Queries 탭 | SQL 모노스페이스 표시, 확장 가능 행, 대기 이벤트 | ✅ |
| 40-6-4 | Locks 탭 | Blocking/Blocked PID, 테이블명, 지속 시간 (>5s 빨강) | ✅ |
| 40-6-5 | Wait Events 탭 | 수평 바 차트 + 테이블 | ✅ |
| 40-6-6 | Backend API | `GET /database/instances, /slow-queries, /locks, /wait-events, /stats` (5개 엔드포인트) | ✅ |

**신규 파일:**
- `frontend/src/app/rum/page.tsx` — RUM 페이지 (Core Web Vitals + Session Replay + Geography)
- `frontend/src/app/golden-signals/page.tsx` — SRE Golden Signals 대시보드
- `frontend/src/app/runtime/page.tsx` — Runtime Monitor 허브 (Python/.NET/Go 선택)
- `frontend/src/app/runtime/python/page.tsx` — Python 3.13 Free-Threaded 모니터링
- `frontend/src/app/runtime/dotnet/page.tsx` — .NET AOT 모니터링
- `frontend/src/app/runtime/go/page.tsx` — Go 1.24 Scheduler Latency 모니터링
- `frontend/src/app/database/page.tsx` — Database Monitoring (쿼리 수준)
- `agent/cmd/collection-server/phase40_api.go` — Phase 40 Backend API (19개 엔드포인트)

**수정된 파일:**
- `frontend/src/components/layout/sidebar.tsx` — RUM, Golden Signals, Runtime, Database 네비게이션 추가
