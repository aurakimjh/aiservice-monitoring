...
> **최종 업데이트**: 2026-03-25 (Session 46 — DIAGNOSTIC_ITEMS_CATALOG 상시 모니터링 메트릭 추가·출처·메트릭ID 컬럼·Cache/MQ ITEM 번호 신규 부여·AI Serving 섹션 추가 + Phase 31 서브태스크 정밀화)
...
[15] Phase 31-1: 내장(🔧) 진단 항목 Go 구현 (ConfigEvidence·LogEvidence·EOSEvidence)  [░░░░░░░░░░]   0%  📋
[16] Phase 31-2: 스크립트(📜) 실행 프레임워크 (Script Executor·스케줄러·Backend 연동)  [░░░░░░░░░░]   0%  📋
[17] Phase 31-3: 수동(🖐️) 트리거 방식 + 고급 진단 (Security·APM·CrossAnalysis·Full)   [░░░░░░░░░░]   0%  📋
[18] Phase 31-4: Java Agent EOL (기능 동등성 검증·마이그레이션)                        [░░░░░░░░░░]   0%  📋
...
| 31-1a | Evidence Collector 인터페이스 | `agent/internal/collector/evidence/` 패키지 — `EvidenceCollector` 인터페이스 정의 | | |
...
| 31-1d | EOSEvidence 플러그인 | `eos-lifecycle-db.json` 기반 버전 체크 — ITEM0068 (17개 제품군) | | |
| 31-1e | Evidence ZIP 생성 + 업로드 | 구조화 JSON → ZIP → HTTPS POST | | |
| 31-1f | 기존 Collector → Evidence 변환 어댑터 | 12종 Collector(os/web/was/db/cache/mq/gpu/ai) 출력을 Evidence JSON으로 변환 | | |
| 31-1g | 🔧 내장 진단 항목 구현 범위 확정 | ITEM0013·0015·0037·0040·0044·0063·0064·0066·0070 — `/proc`/sysfs 기반 설정 스냅샷. 이미 Collector가 상시 수집 중인 항목(📊)은 Evidence 변환 어댑터(31-1f)로 재활용 | | |
| **31-2** | **스크립트(📜) 실행 프레임워크** | Script Executor + 진단 스케줄러 + Backend 연동 | 2~3주 | 📋 |
| 31-2a | Script Executor 구현 | `agent/internal/script/executor.go` — `.sh`/`.ps1`/`.py` 실행 + 타임아웃 + stderr 캡처 + 리소스 제한 | | |
| 31-2b | 진단 스케줄러 | `--mode=diagnose --interval=1d` — cron 기반 자동 실행. ITEM0014·0069 등 delayed 수집 지원 | | |
| 31-2c | 📜 항목 스크립트 매핑 테이블 | ITEM ID ↔ 스크립트 파일명 매핑 — `agent.yaml`의 `diagnostic.scripts` 섹션 (Phase 33 플러그인과 연계) | | |
| 31-2d | Evidence 수신 API | `POST /api/v1/evidence/upload` | | |
| 31-2e | Backend 릴레이 + 자동 진단 트리거 | Collection Server → aitop-backend 포워딩. 수집 완료 → 자동 진단 실행 | | |
| 31-2f | Fleet에 진단 상태 표시 | 마지막 진단 시각, 결과 요약 | | |
| **31-3** | **수동(🖐️) 트리거 방식 + 고급 진단** | UI 트리거 API + 실행 권한 제어 + 고급 진단 플러그인 | 3~4주 | 📋 |
| 31-3a | 🖐️ 항목 UI 트리거 API | `POST /api/v1/agents/{id}/diagnose` — ITEM0030·0049·0059·0222·0224·0226 등 수동 진단 요청. 실행 전 위험도 확인 대화창 표시 | | |
| 31-3b | 실행 권한 및 감사 로그 | 🖐️ 항목은 Admin/SRE 권한만. 실행 기록 + 감사 로그 (컴플라이언스 요건 ITEM0067·0224) | | |
| 31-3c | SecurityEvidence 플러그인 | SSL/TLS·패치·계정 정책 — ITEM0056·0057·0065·0067·0068 | | |
| 31-3d | APMEvidence 플러그인 | 6종 APM SaaS 어댑터 Go 포팅 — ITEM0054 (WhaTap/NR/DD/DT/Scouter/OA) | | |
| 31-3e | CrossAnalysis 플러그인 | IT↔AI 교차 분석용 통합 스냅샷 | | |
| 31-3f | `--mode=collect-only` | 1회 수집 → HTTPS Push → 종료 (에어갭용) | | |
| 31-3g | `--mode=full` | 모니터링 상시 + 진단 스케줄/온디맨드 | | |
| **31-4** | **Java Agent EOL** | 기능 동등성 검증 + 마이그레이션 + 병행 운영 | 4~8주 | 📋 |
| 31-4a | 기능 동등성 테스트 | Java Agent 수집 항목 100% Go Agent 커버 확인 (카탈로그 기준 91건 + 상시 메트릭 35종) | | |
