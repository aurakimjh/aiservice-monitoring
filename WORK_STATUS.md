...
> **최종 업데이트**: 2026-03-21 (Session 16 — AITOP Agent 상세 설계 완료, Phase 15~17 로드맵 추가)
...
═══════════════════════════════════════════════════════════════════════
Phase 15: Agent MVP (F)        [░░░░░░░░░░]   0%  📋  (Agent Core + Collector + 권한검증 + Collection Server + Heartbeat)
...
| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 15-1-1 | Go 프로젝트 초기화 | `aitop-agent/` 디렉토리, go.mod, 기본 구조 (`cmd/`, `internal/`, `pkg/`) | 📋 |
| 15-1-2 | Config Manager | YAML 설정 파일 로딩 (`agent.yaml`), 환경변수 오버라이드, 원격 설정 병합 | 📋 |
| 15-1-3 | Scheduler | cron 표현식 기반 수집 스케줄러, 즉시 실행 트리거 지원 | 📋 |
| 15-1-4 | Health Monitor | 에이전트 자체 CPU/MEM 사용량 모니터링, 자기 진단 | 📋 |
| 15-1-5 | Privilege Checker | 각 Collector 필요 권한 사전 검증, 구조화된 오류 응답 생성 | 📋 |
| 15-1-6 | Sanitizer | API Key / PII 패턴 매칭 마스킹 (정규식 기반) | 📋 |
| 15-1-7 | Local Buffer (SQLite) | 오프라인 시 수집 데이터 로컬 저장, 재연결 시 자동 전송 | 📋 |
| 15-1-8 | 단일 바이너리 빌드 | Linux amd64/arm64 크로스 컴파일, Makefile / Goreleaser 설정 | 📋 |

### 15-2. Collector 인터페이스 + OS Collector — 2~3주차

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 15-2-1 | Collector 인터페이스 정의 | `Collector` interface (Go), `CollectResult`, `CollectError` 구조체 | 📋 |
| 15-2-2 | Collector Registry | 플러그인 등록/탐색/실행 런타임 | 📋 |
| 15-2-3 | OS Collector — CPU/Memory | `/proc/stat`, `/proc/meminfo` 파싱 (Linux) | 📋 |
| 15-2-4 | OS Collector — Disk/Network | `/proc/diskstats`, `/proc/net/dev` 파싱 | 📋 |
| 15-2-5 | OS Collector — Process List | `/proc/[pid]/stat` 프로세스 목록 수집 | 📋 |
| 15-2-6 | OS Collector — System Info | `uname`, `/etc/os-release`, 네트워크 설정 | 📋 |
| 15-2-7 | NDJSON 출력 포맷 | `schema_name`, `schema_version`, `collect_status`, `errors` 포함 출력 구조화 | 📋 |
| 15-2-8 | 권한 부족 시 오류 응답 테스트 | `PERMISSION_DENIED`, `NOT_INSTALLED` 등 각 에러 코드별 응답 검증 | 📋 |
...
### 15-5. AI Collector PoC + 메트릭 연동 — 5~6주차

| # | 작업 | 상세 | 상태 |
|---|------|------|------|
| 15-5-1 | AI-GPU Collector PoC | nvidia-smi 메트릭 수집 (VRAM, 온도, 전력, SM%) | 📋 |
| 15-5-2 | AI-LLM Collector PoC | LLM 설정 파싱, 토큰 사용 로그, Agent 설정 (max_iterations 등) | 📋 |
| 15-5-3 | AI 환경 자동 탐지 | GPU 존재 여부, Python LLM 라이브러리 탐지, VectorDB 프로세스/포트 탐지 | 📋 |
| 15-5-4 | Prometheus Remote Write | 시계열 메트릭(CPU/MEM/GPU 등) → Prometheus 전송 연동 | 📋 |
| 15-5-5 | collect-only 모드 | `--mode=collect-only` 1회 실행 수집 → HTTPS 전송 → 종료 | 📋 |
| 15-5-6 | Agent 설치 패키지 | DEB/RPM 패키지 빌드, systemd 서비스 파일, 설치 스크립트 | 📋 |
