...
| # | 작업 | 상태 | 비고 |
|---|------|:----:|------|
| XL-1 | 복수 서비스 동시 트레이스 조회 | ✅ | Backend: ServiceNames[] IN 쿼리, Frontend: multi-service URL params |
| XL-2 | Jaeger 응답 → 루트 스팬만 XLog 점으로 표시 (하위 스팬 제외) | ✅ | v2 API가 TraceRow(루트 스팬 요약)만 반환하여 자동 충족 |
| XL-3 | Live 모드에서 Jaeger 데이터 없을 때 빈 상태 표시 (데모 폴백 방지) | ✅ | txnSource=live && 0건 → 빈 상태 UI 표시 |
| XL-4 | 서버별 색상 분리 (멀티 서비스 선택 시 서비스별 컬러) | ✅ | serverId=t.service 매핑으로 실데이터에도 적용 |

### WS-3A.2 드래그 선택 + 드릴다운 — 1주

| # | 작업 | 상태 | 비고 |
|---|------|:----:|------|
| XL-5 | XLog 브러시 이벤트 → 트랜잭션 리스트 패널 연동 | ✅ | ECharts brushSelected 핸들러, 기존 구현 확인 |
| XL-6 | HeatMap 셀 클릭 → 해당 시간+레이턴시 구간 트랜잭션 필터링 | ✅ | 셀 좌표 → LATENCY_RANGES 변환, 기존 구현 확인 |
| XL-7 | 선택 영역 표시: "N건 선택됨 (시간 범위, 레이턴시 범위)" | ✅ | 선택 건수 + min~max 응답시간 표시, 기존 구현 확인 |
| XL-8 | 트랜잭션 클릭 → 워터폴 타임라인 바 차트 (현재 테이블 → 바 차트) | ✅ | 인라인 CSS 바 차트 (leftPct/widthPct), 기존 구현 확인 |
| XL-9 | 워터폴에서 스팬 클릭 → 속성 상세 + 트레이스 상세 페이지 이동 | ✅ | 스팬 속성 패널에 "트레이스 상세 →" 링크 추가 |

### WS-3A.3 HeatMap 고도화 — 1주

| # | 작업 | 상태 | 비고 |
|---|------|:----:|------|
| XL-10 | HeatMap 에러 셀 시각 구분 (에러 비율 높은 셀에 ✕ 마커) | ✅ | SVG path ✕ 심볼(size 10), 빨간색, ≥10% 에러율 셀 |
| XL-11 | HeatMap 드래그 선택 → XLog와 동일 트랜잭션 리스트 연동 | ✅ | brushSelected 핸들러, 기존 구현 확인 |
| XL-12 | 시간대별 TPS 오버레이 바 차트 (HeatMap 상단) | ✅ | dual-grid(bar+heatmap), 버킷별 TPS 계산, WhaTap 스타일 |
| XL-13 | 분할 뷰 (XLog + HeatMap 동시 표시) 동기화 | ✅ | 동일 filteredTransactions + range 공유, 선택 상태 동기화 |
...
| # | 작업 | 상태 | 비고 |
|---|------|:----:|------|
| R-15 | 전 문서의 성능 수치에 출처 태그 추가 (실측/PoC/추정/설계목표) | ✅ | §5.3 — SOLUTION_STRATEGY.md §1.2, §5.4, §9.2에 `[설계목표]`/`[경쟁사 공개자료]` 태그 추가 |
| R-16 | "Datadog 수준", "세계 최고", "글로벌 표준" 표현에 증빙 근거 연결 | ✅ | §3.3 — SOLUTION_STRATEGY.md v1.3 AI Observability 표 및 §5.4 경쟁사 비교에 근거 연결 |
| R-17 | 즉시 측정 가능한 벤치마크 항목 목록 정의 (ingest, UI P95, agent overhead) | ✅ | §3.3 — V1_RELEASE_CRITERIA.md §7에 필수 실측 항목 및 측정 도구 정의 |
